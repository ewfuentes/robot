...
PYBIND11_MODULE(so3_python, m) {
    py::class_<SO3>(m, "SO3")
        ...
        .def_static("from_quat", [](const Eigen::Vector<double, 4> &quat) -> SO3 { 
            const Eigen::Quaterniond eigen_quat(quat(3), quat(0), quat(1), quat(2));
            return SO3(eigen_quat); 
        })
        .def_static("from_matrix", [](const Eigen::Matrix3d &rot) -> SO3 { 
            return SO3(rot); 
        })
        .def_static("exp", [](const Eigen::Vector3d &tangent_vec) -> SO3 { return SO3::exp(tangent_vec); })
