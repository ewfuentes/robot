...
**214.0°** (`m7_build_pairing_requests.DEFAULT_MOUNT_OFFSET_DEG`).
...
- `bearing_span_from()` — the angular interval a candidate subtends, from its
  ...
  centroid range.
- `wedge_candidates()` — spatial gate; an extended candidate qualifies if its
  *span* overlaps the wedge, so an island is not rejected for its centroid.
...
### M7 — `m7_build_pairing_requests`
Builds Gemini label requests pairing merged landmarks (Set 1) against map
candidates (Set 2), for training the correspondence model. Mirrors LOCI's
`landmark_pairing_cli` — same system prompt shape, same hard/easy negative
definitions, same uniqueness 1–5 score, same `CorrespondencePair` parsing —
and changes only how Set 2 is gathered: **bearing wedge** instead of satellite
tile.

Also writes wedge maps (`figures/*.png`), the exact filled prompts
(`prompts/*.txt`), and a review page.

| flag | default | meaning / why |
|---|---|---|
| `--mount_offset_deg` | 214.0 | camera→body yaw; **re-derive per leg** |
| `--bearing_slack_deg` | 25.0 | wedge half-width added to the tracklet's own angular width. Set **8** once the offset is calibrated; the wide default exists for uncalibrated legs |
| `--min_supports` | 5 | landmark support floor |
| `--min_observations` | 3 | **fused bearings needed to triangulate.** Below 3 the wedge is unconstrained — one 2-observation tracklet pulled 66,052 candidates — and any label it earns is geometrically meaningless |
| `--max_set2` | 500 | candidates shown per prompt. See §3.5 |
| `--max_bearing_residual_deg` | none | optional gate on triangulation residual |
| `--max_tracklets` | none | cap, for quick iteration |
| `--thinking_level` | LOW | Gemini thinking level |

Slack matters enormously: at 25° a downtown wedge returns ~71,000 candidates
(half the catalog — i.e. no gating at all); at 8° with the calibrated offset,
~12,600. Calibrate, then tighten.

**Set 2 ordering** is `set2_salience()` — a *score*, not tiers:
ENC +3, "visually conspicuous" +3, tall-structure class +2.5, salient key +1,
named +1.5, plus up to +3 for actual height (`max(height, levels·3.5)/60`).
Tiered ordering produced two cliff failures in a row (§3.5). Ordering
**ignores Set 1 on purpose**: ranking candidates by similarity to the query
would bias the labels toward whatever the baseline scorer already believes.

`TALL_STRUCTURE_VALUES` (crane, lighthouse, water_tower, storage_tank, chimney,
tower, mast, silo, gasometer, windmill, obelisk, bridge) exists because OSM
tags a 50–80 m container crane as bare `man_made=crane` with no height.

Prompt output adds `match_type` per match: **instance** (this exact object) vs
**category** (right kind, cannot say which). The prompt states explicitly that
these are not degrees of confidence.
...
### M8 — `m8_pairing_results_viewer`
Renders pairing labels: Set 1 bundle, wedge map, every proposed match against
**the Set 2 bundle the model actually saw**, plus hard/easy negatives and the
uniqueness score. `match_type` is a first-class column. Tolerates both the
plain-int and `{set_2_id, match_type}` forms of `set_2_matches`.
