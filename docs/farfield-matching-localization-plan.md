...
- Build `OdometryDelta` per keyframe from `frames_gps.csv` via the
  `gps_to_odometry` producer (design doc §5.2, Milestone 4): body-frame SE(2)
  increments — the differenced ENU fix (`ingest.fill_enu` already computes
  ENU positions) rotated into body frame by course over ground, `dyaw_rad`
  from differenced course. Speed gating and gap budgets are producer-internal
  (slow steps emit `dyaw = 0` with inflated `sigma_yaw_rad`). The
  derived-data caveats (`left_m ≈ 0` by construction, course-grade yaw
  pessimistic vs a real gyro) are §5.2's.
...
the ground truth here. With GPS-derived (hence low-noise) increments and a
tight prior, dead reckoning alone will track well — so a small final position
error does *not* demonstrate that the landmarks are helping. In this configuration
...
inject drift/noise via the producer's noise-injection knob to emulate
IMU-grade odometry, widen to a loose or global prior, and compare against
the dead-reckoning-only baseline. Deployment has no GPS at all — the derived
increments stand in for an IMU (design doc §5.2) — so the drift-injected run
is closer to the real problem than the clean one.
