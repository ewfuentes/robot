...
`harbor_osm_enc_trimmed_v1.feather`. On leg 1 that is 176 requests over 7,950
...
Submit, then aggregate:

```bash
bazel run //…swag/scripts:vertex_batch_manager -- run-online \
...
6. **`instance_max_rows`** in `m9_match_landmarks` (default 5) — the ceiling
