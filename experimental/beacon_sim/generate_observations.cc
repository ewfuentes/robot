...
namespace experimental::beacon_sim {
  std::optional<BeaconObservation> generate_observation(const Beacon &beacon, const RobotState &robot, const ObservationConfig &) {
    ...
    return BeaconObservation{.maybe_id = beacon.id, .maybe_range_m = range_m, .maybe_bearing_rad = bearing_rad};
  }
