...
#include "planning/david_planning_one.hh"
#include "planning/david_planning_two.hh"
...
//David's Planner One config
PlannerResult run_planner(const planning::RoadMap &road_map, const EkfSlam &ekf,
                          [[maybe_unused]] const BeaconPotential &beacon_potential,
                          [[maybe_unused]] const proto::DavidPlannerOne &config,
                          const double max_sensor_range_m,
                          const std::optional<time::RobotTimestamp::duration> &timeout) {
    const planning::DavidPlannerConfigOne options = 
        {
            .max_visits = 1,
            .max_plans = 70,
            .timeout = timeout,
    ...
    const time::RobotTimestamp start_time = time::current_robot_time();
    const auto maybe_plan = david_planner_one<int>(successor_func,termination_check,road_map,options,est,beacon_potential,ekf,config.favor_goal());
    const time::RobotTimestamp::duration elapsed_time = time::current_robot_time() - start_time;
...
//David's Planner Two config
PlannerResult run_planner(const planning::RoadMap &road_map, const EkfSlam &ekf,
                          [[maybe_unused]] const BeaconPotential &beacon_potential,
                          [[maybe_unused]] const proto::DavidPlannerTwo &config,
                          const double max_sensor_range_m,
                          const std::optional<time::RobotTimestamp::duration> &timeout) {
    const planning::DavidPlannerConfigTwo options = 
        {
            .max_visits = 1,
            .max_plans = 70,
            .timeout = timeout,
    ...
    const time::RobotTimestamp start_time = time::current_robot_time();
    const auto maybe_plan = david_planner_two<int>(successor_func,termination_check,road_map,options,est,beacon_potential,ekf,config.favor_goal());
    const time::RobotTimestamp::duration elapsed_time = time::current_robot_time() - start_time;
...
//David's Planner Three config
PlannerResult run_planner(const planning::RoadMap &road_map, const EkfSlam &ekf,
                          [[maybe_unused]] const BeaconPotential &beacon_potential,
                          [[maybe_unused]] const proto::DavidPlannerThree &config,
                          const double max_sensor_range_m,
                          const std::optional<time::RobotTimestamp::duration> &timeout) {
    const planning::DavidPlannerConfigThree options = 
        {
            .max_visits = 1,
            .max_plans = 70,
            .max_sensor_range_m = max_sensor_range_m,
    ...
    auto successor_func = [&road_map](const int &node_idx) -> std::vector<planning::SuccessorThree<int>> {
        std::vector<planning::SuccessorThree<int>> out;
        const std::vector<std::tuple<int, Eigen::Vector2d>> &vector = road_map.neighbors(node_idx);
    ...
    const time::RobotTimestamp start_time = time::current_robot_time();
    const auto maybe_plan = david_planner_three<int>(successor_func,road_map,options,est,beacon_potential,ekf,config.favor_goal());
    const time::RobotTimestamp::duration elapsed_time = time::current_robot_time() - start_time;
        ...
        pool.detach_task([=, &remaining, eval_base_seed = config.evaluation_base_seed(),
            ...
            for (const auto &planner_config : config.planner_configs()) {
                ...
                switch (planner_config.planner_config_oneof_case()) {
                    ...
                    case proto::PlannerConfig::kDavidPlannerOneConfig: {
                        results[planner_config.name()] = run_planner(
                            road_map, ekf, world_map.beacon_potential(),
                            planner_config.david_planner_one_config(), max_sensor_range_m, timeout);    
                        break;
                    }
                    case proto::PlannerConfig::kDavidPlannerTwoConfig: {
                        results[planner_config.name()] = run_planner(
                            road_map, ekf, world_map.beacon_potential(),
                            planner_config.david_planner_two_config(), max_sensor_range_m, timeout);    
                        break;
                    }
                    case proto::PlannerConfig::kDavidPlannerThreeConfig: {
                        results[planner_config.name()] = run_planner(
                            road_map, ekf, world_map.beacon_potential(),
                            planner_config.david_planner_three_config(), max_sensor_range_m, timeout);    
                        break;
                    }
                    default: {
