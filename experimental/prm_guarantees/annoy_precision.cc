...
int precision(int f = 40, int n = 1000000) {
    ...
    for (int i = 0; i < prec_n; ++i) {
        ...
        for (std::vector<int>::iterator limit = limits.begin(); limit != limits.end(); ++limit) {
            t_start = std::chrono::high_resolution_clock::now();
            t.get_nns_by_item(j, (*limit), -1, &toplist,
                              nullptr);  // search_k defaults to "n_trees * n" if not provided.
            t_end = std::chrono::high_resolution_clock::now();
            auto duration =
            ...
            std::vector<int>::iterator it_set =
                std::set_intersection(closest.begin(), closest.end(), toplist.begin(),
                                      toplist.end(), intersection.begin());
