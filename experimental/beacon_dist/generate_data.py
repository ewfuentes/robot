...
def image_from_letter_set(letter_set: dict[str, LetterPosition], height, width) -> np.ndarray:
    with Image(width=width, height=height, background=Color('#fff')) as img:
        with Drawing() as ctx:
            ctx.font_size = 100
            for s, position in letter_set.items():
                img.annotate(s, ctx, left=position.y, baseline=position.x, angle=position.angle)
        return np.array(img)
...
def generate_data(idx):
    ...
    return serialize_keypoints_and_descriptors(idx, keypoints, descriptors, class_labels)
