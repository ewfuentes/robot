...
void write_priors(const lrn_desc::FourSeasonsParser& parser,
                  robot::InOut<robot::sqlite3::Database> colmap_db, bool groundtruth_priors_only) {
    ...
    for (size_t i = 0; i < parser.num_images(); i++) {
        ...
        if (groundtruth_priors_only && img_pt.world_from_cam_ground_truth()) {
            if (!p_first_in_world) {
                p_first_in_world = img_pt.world_from_cam_ground_truth()->translation();
                break;
            }
        } else if (!groundtruth_priors_only && img_pt.cam_in_world()) {
            if (!p_first_in_world) {
                p_first_in_world = *img_pt.cam_in_world();
                break;
            }
        }
    }
    ...
    for (size_t i = 0; i < parser.num_images(); i++) {
        ...
        if (groundtruth_priors_only && img_pt.world_from_cam_ground_truth()) {
            ...
            std::cout << "grnd_trth translation: "
                      << img_pt.world_from_cam_ground_truth()->translation() << std::endl;
            ...
            std::cout << "gps translation: " << *img_pt.cam_in_world() << std::endl;
...
int main(int argc, const char** argv) {
    ...
    options.add_options()
        ...
        ("use_groundtruth_priors_only", "Flag to use groundtruth location prior only. Otherwise, will use gps location prior only.", 
    ...
    ROBOT_CHECK(std::filesystem::exists(path_colmap_database));
    ...
    write_priors(parser, robot::make_in_out<robot::sqlite3::Database>(colmap_db),
                 args["use_groundtruth_priors_only"].as<bool>());
}