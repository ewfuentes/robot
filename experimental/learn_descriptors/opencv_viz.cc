...
void viz_scene(const std::vector<Eigen::MatrixXd> &poses,
    ...
    for (unsigned int i = 0; i < poses.size(); i++) {
        const Eigen::MatrixXd &pose = poses[i];
        cv::Affine3d cv_pose(eigen_to_cv_mat(pose.block(0, 0, 3, 3)),
                             cv::Affine3d::Vec3{pose(0, 3), pose(1, 3), pose(2, 3)});
        window.showWidget("pose_" + std::to_string(i), cv::viz::WCoordinateSystem(pose_size),
                          cv_pose);
    }
    ...
    for (unsigned int i = 0; i < points.size(); i++) {
        ...
        window.showWidget("translation_" + std::to_string(i),
