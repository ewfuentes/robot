...
class GPSParserHelper {
   public:
    static size_t gps_utc_to_unix_time(const nmea::date& utc_date,
                                       const double utc_time_day_seconds) {
...
FourSeasonsParser::FourSeasonsParser(const std::filesystem::path& root_dir,
    ...
    TimeDataMap gnss_poses_time_map =
        TxtParserHelper::create_gnss_poses_time_data_map(path_gnss, min_time_sig_figs);
    TimeDataMap vio_poses_time_map =
    ...
    for (const std::pair<size_t, std::vector<std::string>>& pair_time_data : img_time_list) {
        ...
        if (gnss_poses_time_map.find(time_key) != gnss_poses_time_map.end()) {
            ...
            Eigen::Vector3d t_gps_cam_from_AS_w(
                std::stod(
                    parsed_line_gnss_poses[static_cast<size_t>(TxtParserHelper::GPSIdx::TRAN_X)]),
                std::stod(
                    parsed_line_gnss_poses[static_cast<size_t>(TxtParserHelper::GPSIdx::TRAN_Y)]),
                std::stod(
                    parsed_line_gnss_poses[static_cast<size_t>(TxtParserHelper::GPSIdx::TRAN_Z)]));
            Eigen::Quaterniond R_gps_cam_from_AS_w(
                std::stod(
                    parsed_line_gnss_poses[static_cast<size_t>(TxtParserHelper::GPSIdx::QUAT_W)]),
                std::stod(
                    parsed_line_gnss_poses[static_cast<size_t>(TxtParserHelper::GPSIdx::QUAT_X)]),
                std::stod(
                    parsed_line_gnss_poses[static_cast<size_t>(TxtParserHelper::GPSIdx::QUAT_Y)]),
                std::stod(
                    parsed_line_gnss_poses[static_cast<size_t>(TxtParserHelper::GPSIdx::QUAT_Z)]));
        ...
        if (vio_poses_time_map.find(time_key) != vio_poses_time_map.end()) {
            ...
            Eigen::Vector3d t_AS_w_from_vio_cam(
                std::stod(parsed_line_vio[static_cast<size_t>(TxtParserHelper::ResultIdx::TRAN_X)]),
                std::stod(parsed_line_vio[static_cast<size_t>(TxtParserHelper::ResultIdx::TRAN_Y)]),
                std::stod(
                    parsed_line_vio[static_cast<size_t>(TxtParserHelper::ResultIdx::TRAN_Z)]));
            Eigen::Quaterniond R_AS_w_from_vio_cam(
                std::stod(parsed_line_vio[static_cast<size_t>(TxtParserHelper::ResultIdx::QUAT_W)]),
                std::stod(parsed_line_vio[static_cast<size_t>(TxtParserHelper::ResultIdx::QUAT_X)]),
                std::stod(parsed_line_vio[static_cast<size_t>(TxtParserHelper::ResultIdx::QUAT_Y)]),
                std::stod(
                    parsed_line_vio[static_cast<size_t>(TxtParserHelper::ResultIdx::QUAT_Z)]));
