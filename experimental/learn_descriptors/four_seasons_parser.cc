...
class GPSParserHelper {
   public:
    static size_t gps_utc_to_unix_time(const nmea::date& utc_date,
                                       const long utc_time_day_seconds) {
        ...
        std::chrono::nanoseconds utc_time_day_ns = std::chrono::seconds{utc_time_day_seconds};
        std::chrono::sys_time<std::chrono::nanoseconds> timestamp = date + utc_time_day_ns;
        return timestamp.time_since_epoch().count();
    }
    using TimeGPSList = std::vector<std::pair<size_t, lrn_descs::ImagePoint::GPSData>>;
    static TimeGPSList create_gps_time_data_map(const std::filesystem::path& path_gps) {
        TimeGPSList time_map_gps;
        std::ifstream file_gps(path_gps);
        std::string line;
        std::optional<nmea::sentence> nmea_sentence;
        nmea::date date_last;
        size_t time_of_day_last = 0;
                ...
                if (gga.utc.exists() && gga.latitude.exists() && gga.longitude.exists()) {
                    ...
                    if (gga.utc.get() == time_of_day_last) {  // GGA messages for this dataset come
                        ...
                        time_map_gps.push_back(std::make_pair(
                            gps_utc_to_unix_time(date_last, gga.utc.get()), gps_data));
                    }
                }
...
FourSeasonsParser::FourSeasonsParser(const std::filesystem::path& root_dir,
    ...
    : root_dir_(root_dir),
      ...
      cal_(TxtParserHelper::load_camera_calibration(calibration_dir)),
    ...
    const size_t min_time_sig_figs = TxtParserHelper::min_sig_figs_result_time(path_vio);
    TimeDataList img_time_list =
        TxtParserHelper::create_img_time_data_list(path_img, min_time_sig_figs);
    TimeDataMap reference_time_map =
        TxtParserHelper::create_reference_time_data_map(path_reference, min_time_sig_figs);
    TimeDataMap vio_time_map =
        TxtParserHelper::create_vio_time_data_map(path_vio, min_time_sig_figs);
    GPSParserHelper::TimeGPSList gps_time_list =
        GPSParserHelper::create_gps_time_data_map(path_gps);
    ...
    for (const std::pair<size_t, std::vector<std::string>>& pair_time_data : img_time_list) {
        ...
        img_pt.seq = std::stoull(
            pair_time_data.second[static_cast<size_t>(TxtParserHelper::ImgIdx::TIME_NS)]);
        ...
        if (reference_time_map.find(time_key) != reference_time_map.end()) {
            ...
            Eigen::Vector3d gps_translation(
                std::stod(
                    parsed_line_reference[static_cast<size_t>(TxtParserHelper::GPSIdx::TRAN_X)]),
                std::stod(
                    parsed_line_reference[static_cast<size_t>(TxtParserHelper::GPSIdx::TRAN_Y)]),
                std::stod(
                    parsed_line_reference[static_cast<size_t>(TxtParserHelper::GPSIdx::TRAN_Z)]));
            Eigen::Quaterniond gps_quat(
                std::stod(
                    parsed_line_reference[static_cast<size_t>(TxtParserHelper::GPSIdx::QUAT_W)]),
                std::stod(
                    parsed_line_reference[static_cast<size_t>(TxtParserHelper::GPSIdx::QUAT_X)]),
                std::stod(
                    parsed_line_reference[static_cast<size_t>(TxtParserHelper::GPSIdx::QUAT_Y)]),
                std::stod(
                    parsed_line_reference[static_cast<size_t>(TxtParserHelper::GPSIdx::QUAT_Z)]));
        ...
        if (vio_time_map.find(time_key) != vio_time_map.end()) {
            ...
            Eigen::Vector3d ground_truth_translation(
                std::stod(parsed_line_gps[static_cast<size_t>(TxtParserHelper::ResultIdx::TRAN_X)]),
                std::stod(parsed_line_gps[static_cast<size_t>(TxtParserHelper::ResultIdx::TRAN_Y)]),
                std::stod(
                    parsed_line_gps[static_cast<size_t>(TxtParserHelper::ResultIdx::TRAN_Z)]));
            Eigen::Quaterniond ground_truth_quat(
                std::stod(parsed_line_gps[static_cast<size_t>(TxtParserHelper::ResultIdx::QUAT_W)]),
                std::stod(parsed_line_gps[static_cast<size_t>(TxtParserHelper::ResultIdx::QUAT_X)]),
                std::stod(parsed_line_gps[static_cast<size_t>(TxtParserHelper::ResultIdx::QUAT_Y)]),
                std::stod(
                    parsed_line_gps[static_cast<size_t>(TxtParserHelper::ResultIdx::QUAT_Z)]));
        ...
        while (time_key > gps_time_list[gps_idx].first &&
        ...
        if (time_key - gps_time_list[gps_idx].first <= (1.0 / imu_hz) * 1e9) {
...
liegroups::SE3 FourSeasonsParser::FourSeasonsTransforms::get_transform_from_line(
    ...
    std::vector<std::string> parsed_transform_line = TxtParserHelper::parse_line_adv(line, ",");
