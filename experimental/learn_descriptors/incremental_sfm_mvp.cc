...
int main() {
    ...
    //         .finished();
    // cv::Mat K_mat = (cv::Mat_<double>(3, 3) << K->fx(), 0, K->px(), 0, K->fy(), K->py(), 0, 0,
    // 1); cv::Mat D_mat = (cv::Mat_<double>(5, 1) << D(0, 0), D(1, 0), D(2, 0), D(3, 0), D(4, 0));
    ...
    //     Eigen::Isometry3d T_world_cam = world_from_boat *
    //     DataParser::get_boat_from_camera(img_pt); gtsam::Pose3
    //     T_world_cam_gtsam(T_world_cam.matrix()); cam_pose.emplace(id, T_world_cam_gtsam);
    ...
    //     std::vector<gtsam::Pose3>{cam_id_pose.second});
    ...
    //         if (lmk_id_map.find(key) != lmk_id_map.end()) {
    //             auto id = lmk_id_map.at(key);
    //             if (lmk_triangulated_map_filtered.find(id) ==
    ...
    //     detail::averagePoses(sym_pose.second));
    // }
