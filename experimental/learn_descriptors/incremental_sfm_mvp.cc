...
static gtsam::Values optimize_graph(const gtsam::NonlinearFactorGraph &graph,
    ...
    std::function<void(const gtsam::Values &, const epoch, const std::vector<gtsam::Symbol> &,
        ...
        graph_itr_debug_func = [&](const gtsam::Values &vals, const epoch iter,
            ...
            detail::graph_values(vals, window_name, symbols_pose, symbols_landmarks);
        };
...
static gtsam::Point3 get_variance(const std::vector<gtsam::Point3> &points) {
    const gtsam::Point3 mean = detail::averagePoints(points);
...
int main(int argc, const char **argv) {
    ...
    const std::vector<int> indices{100, 120, 140, 160, 180};
    ...
    FrameId id = 0;
    std::unordered_map<FrameId, gtsam::Pose3> poses_cam_init;
    std::vector<Eigen::Isometry3d> cam_references;
    ...
    for (const int &idx : indices) {
        ...
        cv::fisheye::undistort(img, img_undistorted, K_mat, D_mat);
        ...
        if (id == 0) {
            ...
            world_from_cam.translation() =
                (parser.w_from_gpsw() * parser.e_from_gpsw().inverse()).matrix() * p_gps_in_ECEF;

            Eigen::
        }
        poses_cam_init.emplace(id, world_from_cam.matrix());

        if (img_pt.AS_w_from_gnss_cam) {
            cam_references.push_back(
    ...
    geometry::viz_scene(cam_references, std::vector<Eigen::Vector3d>(), true, true, "References");

    // populate feature_tracks and lmk_id_map
    // TODO: exhaustively get matches
    ...
    for (const std::pair<LandmarkId, FeatureTrack> lmk_feat : feature_tracks) {
        ...
        for (const std::pair<FrameId, KeypointCV> &feat_track : feature_track.obs_) {
            feat_cam_poses.push_back(poses_cam_init[feat_track.first]);
            feat_kpts.push_back(gtsam::Point2(feat_track.second.x, feat_track.second.y));
        }
        bool triangulate_success =
            detail::attempt_triangulate(feat_cam_poses, feat_kpts, K, p_wrld_kpt);
    ...
    geometry::viz_scene(std::vector<Eigen::Isometry3d>(), triangulated_lmks, true, true,
                        "Unfiltered points");
    const gtsam::Point3 variance_pts = detail::get_variance(triangulated_lmks);
    const gtsam::Point3 std_dev_pts = variance_pts.array().sqrt().matrix();
    const gtsam::Point3 mean_pts = detail::averagePoints(triangulated_lmks);
    ...
    std::cout << "filtered variance " << detail::get_variance(filtered_points) << std::endl;
    ...
    graph_.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(gtsam::Symbol('x', 0),
                                                            poses_cam_init.at(0), prior_pose_noise);
    initial_estimate_.insert_or_assign(gtsam::Symbol('x', 0), poses_cam_init.at(0));
    symbols_pose.push_back(gtsam::Symbol('x', 0));
    symbols_poses_values_iter.emplace(gtsam::Symbol('x', 0),
                                      std::vector<gtsam::Pose3>{poses_cam_init.at(0)});
    for (const std::pair<FrameId, gtsam::Pose3> cam_id_pose : poses_cam_init) {
    ...
    // detail::graph_values(initial_estimate_, "Confirmation", symbols_pose,
    // symbols_landmarks);

    // do local optimizations and add to iter cache
    // const int window = 2;
    std::cout << "length of cam_poses: " << poses_cam_init.size() << std::endl;
    for (const auto &id_pose : poses_cam_init) {
    ...
    for (size_t i = 0; i < indices.size() - 1; i++) {
        ...
        local_graph_.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
            gtsam::PriorFactor<gtsam::Pose3>(poses[0], poses_cam_init.at(i), prior_pose_noise));
        std::cout << "fuck" << std::endl;
        local_graph_.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
            gtsam::PriorFactor<gtsam::Pose3>(poses[1], poses_cam_init.at(i + 1), prior_pose_noise));
        std::cout << "mega fuck" << std::endl;
        local_estimate_.insert_or_assign(poses[0], poses_cam_init.at(i));
        local_estimate_.insert_or_assign(poses[1], poses_cam_init.at(i + 1));

        std::vector<cv::DMatch> matches =
            frontend.get_matches(frames[i].get_descriptors(), frames[i + 1].get_descriptors());
        frontend.enforce_bijective_matches(matches);
        std::vector<gtsam::Pose3> feat_cam_poses{poses_cam_init.at(i), poses_cam_init.at(i + 1)};
        ...
        const gtsam::Values symbols_result_local = detail::optimize_graph(
    ...
    for (std::pair<gtsam::Symbol, std::vector<gtsam::Pose3>> sym_pose : symbols_poses_values_iter) {
        initial_estimate_.insert_or_assign(sym_pose.first, detail::averagePoses(sym_pose.second));
    }
    for (std::pair<gtsam::Symbol, std::vector<gtsam::Point3>> sym_lmk :
         symbols_landmarks_values_iter) {
        initial_estimate_.insert_or_assign(sym_lmk.first, detail::averagePoints(sym_lmk.second));
    }

    // do global optimization
    const gtsam::Values result =
        detail::optimize_graph(graph_, initial_estimate_, symbols_pose, symbols_landmarks);
    std::cout << "about to visualize result" << std::endl;
    detail::graph_values(result, "Result", symbols_pose, symbols_landmarks);
}