        ...
        if (j.contains("sensor")) {
            ...
            if (sensor_data["type"] == "accelerometer") {
                out.accel.emplace_back(AccelMeasurement{
                    ...
                    .accel_mpss = Eigen::Vector3d{v.at(0).get<double>(), v.at(1).get<double>(), v.at(2).get<double>()},
                });
            } else if (sensor_data["type"] == "gyroscope") {
                out.gyro.emplace_back(GyroMeasurement{
                    .time_of_validity = time_of_validity,
                    .angular_vel_radps = Eigen::Vector3d{v.at(0).get<double>(), v.at(1).get<double>(), v.at(2).get<double>()},
                });
            }
            ...
            std::array<FrameCalibration, 2> calibration;
            {
                ...
                calibration[0].focal_length = {
                    calib_info["focalLengthX"].get<double>(),
                    calib_info["focalLengthY"].get<double>()
                }; 
                calibration[0].principle_point = {
                    calib_info["principalPointX"].get<double>(),
                    calib_info["principalPointY"].get<double>()
                }; 
                calibration[0].exposure_time = time::as_duration(frame["exposureTimeSeconds"].get<double>());
            }
            {
                ...
                calibration[1].focal_length = {
                    calib_info["focalLengthX"].get<double>(),
                    calib_info["focalLengthY"].get<double>()
                }; 
                calibration[1].principle_point = {
                    calib_info["principalPointX"].get<double>(),
                    calib_info["principalPointY"].get<double>()
                }; 
