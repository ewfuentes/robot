...
class CityPbfMapTest(unittest.TestCase):
    ...
    def test_resolve_pbf_searches_dirs_in_order(self):
        with tempfile.TemporaryDirectory() as t1, tempfile.TemporaryDirectory() as t2:
            ...
            (d2 / "netherlands-250101.osm.pbf").write_text("")
            resolved = city_pbf_map.resolve_pbf("netherlands_norr", [d1, d2])
            self.assertEqual(resolved, d2 / "netherlands-250101.osm.pbf")

    def test_resolve_pbf_falls_back_to_first_dir_when_missing(self):
        with tempfile.TemporaryDirectory() as t1, tempfile.TemporaryDirectory() as t2:
            d1, d2 = Path(t1), Path(t2)
            resolved = city_pbf_map.resolve_pbf("netherlands_veluwe", [d1, d2])
            # Neither exists; we return the first dir so the error message
            # points somewhere concrete.
            self.assertEqual(resolved, d1 / "netherlands-250101.osm.pbf")
