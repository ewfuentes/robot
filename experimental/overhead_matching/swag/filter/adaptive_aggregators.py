...
def _load_similarity_matrix(path: Path, device: torch.device) -> torch.Tensor:
    ...
    The device parameter is kept for API compatibility but currently unused
    for similarity matrices (the output log-likelihoods will be on the
    same device as the input similarity row).
    """
    del device  # Kept on CPU to save GPU memory
...
class SingleSimilarityMatrixAggregator(ObservationLogLikelihoodAggregator):
    ...
    def from_config(
        ...
        similarity_matrix = _load_similarity_matrix(
            config.similarity_matrix_path, device
        )
        return cls(similarity_matrix, panorama_metadata, config.sigma, device)
...
class ImageLandmarkPrivilegedInformationFusion(ObservationLogLikelihoodAggregator):
    ...
    def from_config(
        ...
        image_sim = _load_similarity_matrix(
            config.image_similarity_matrix_path, device
        )
        landmark_sim = _load_similarity_matrix(
            config.landmark_similarity_matrix_path, device
        )
        return cls(
            image_sim,
            landmark_sim,
...
def aggregator_from_config(
    ...
    if isinstance(config, SingleSimilarityMatrixAggregatorConfig):
        return SingleSimilarityMatrixAggregator.from_config(
            config, panorama_metadata, device
        )
