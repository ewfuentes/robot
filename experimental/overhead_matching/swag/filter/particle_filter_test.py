...
from particle_filter import (
    wag_motion_model,
    wag_observation_liklihood_from_similarity_matrix,
    wag_update_particle_weights,
...
class TestParticleFilter(unittest.TestCase):
    ...
    def test_wag_observation_liklihood_from_similarity_matrix(self):
        ...
        liklihood = wag_observation_liklihood_from_similarity_matrix(
            similarity_matrix, sigma
        )

        # Check shape
        self.assertEqual(liklihood.shape, similarity_matrix.shape)

        # Check that maximum similarity gives max likelihood
        max_sim_idx = torch.argmax(similarity_matrix)
        max_lik_idx = torch.argmax(liklihood)
        self.assertEqual(max_sim_idx, max_lik_idx)

        # Check that values are positive
        self.assertTrue(torch.all(liklihood > 0))

    def test_wag_update_particle_weights(self):
        observation_liklihood_matrix = torch.tensor([[0.1, 0.2], [0.3, 0.4]])
        ...
        weights = wag_update_particle_weights(
            observation_liklihood_matrix, patch_positions, particles
        )

        # Check shape
        self.assertEqual(weights.shape, (particles.shape[0],))

        # Check that weights sum to 1
        self.assertTrue(torch.isclose(torch.sum(weights), torch.tensor(1.0)))
    ...
    def test_wag_multinomial_resampling(self):
        ...
        # Check shape
        self.assertEqual(resampled.shape, particles.shape)
