...
def wag_motion_model(particles: torch.Tensor,
    ...
    """
    NOTE: This motion model does not add noise to dimensions that have zero motion.
        E.g., if motion is [1, 0], no noise is added to the Y dimension
    ...
    delta_t = motion_delta
    n_t = delta_t.unsqueeze(0) * noise_percent * torch.normal(0, 1, size=(n_particles, n_dim), generator=generator)
    return particles + delta_t.unsqueeze(0) + n_t  ## NOTE: this differs from the thesis, but I think the thesis Equation 3.5 is wrong
...
def wag_observation_liklihood_from_similarity_matrix(
    ...
    return (
        1 / (torch.sqrt(torch.tensor(2 * torch.pi)) * sigma) *
        torch.exp(
            -0.5 * torch.square(
...
def wag_update_particle_weights(observation_liklihood_matrix: torch.Tensor,
    ...
    particle_liklihood = observation_liklihood_matrix.flatten()[torch.argmin(dist, dim=1)]
    # normalize
...
def wag_multinomial_resampling(particles: torch.Tensor,
    ...
    generator: random generator
    """
    norm_cumsum = torch.cumsum(weights, dim=0) / weights.sum()
