...
The pipeline writes six viewers into a run directory and until now they were
only reachable by knowing the path. This walks the run, reports what each
stage actually produced, and links them in pipeline order so a run can be
browsed end to end.
...
STAGES = [
    ...
    ("M7", "wedge maps", "pairing/index.html",
     "bearing wedges, Set 2 candidates, filled pairing prompts"),
    ("M8", "pairing labels", "pairing/review/index.html",
     "instance/category matches and negatives for model training"),
]
...
def summarise(run_dir: Path) -> dict:
    ...
    pairing = run_dir / "pairing" / "requests.jsonl"
    if pairing.exists():
        n = sum(1 for line in open(pairing) if line.strip())
        out["wedge maps"] = f"{n} tracklets"
    labels = run_dir / "pairing" / "results.jsonl"
    if labels.exists():
        n = sum(1 for line in open(labels) if line.strip())
        out["pairing labels"] = f"{n} labelled"
    return out
