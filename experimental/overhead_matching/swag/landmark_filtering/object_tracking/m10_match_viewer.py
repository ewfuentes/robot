...
frame it carries:
  ...
  - a bearing ray from each of those anchors, in the world frame the filter
    uses: `bearing_world = heading + bearing_body`, taken from
    `filter._mixture_block_log_terms` (which zeroes
    `bearing_world - heading - observed`) rather than restated here, with
    compass bearings clockwise from north per `geodesy.compass_bearing_rad`;
  - every map row the match expanded to.

A ray that misses its matched landmark is a wrong match, and nothing in the
JSON shows that as fast as one look at the geometry. The bearings are the same
numbers m11 exports -- same `body_frame_measurements`, same resolved mount
offset -- so what is drawn here is what the filter is fed.

Everything is embedded in the page: the basemap is vector geometry pulled from
the dataset's own landmark feather, so there is no tile host and no network,
and the file stays a frozen record that can be copied elsewhere and still
render.
...
def source_links(landmark_id, lm, meta_by_track, range_name):
...
def stats_line(key: str, entry: dict, lm: dict, uniqueness: dict,
               log_lr_defaults: dict) -> str:
    ...
    downgraded to a category, and its own `uniqueness_score` -- which m9
    requires in the response schema and then stores nowhere, so it is read back
    out of `results.jsonl`. `default_log_lr` is the filter's floor: it is what
    every landmark *not* named here scores, and a named landmark's weight is
    the gap to it.
    """
    ...
    bits.append(f"{lm.get('n_supports', '?')} supports")
    bits.append(f"tracks {lm.get('track_ids', [])}")
...
def load_log_lrs(match_dir: Path) -> tuple:
    ...
    log likelihood ratio m9 derived from it, with every unlisted landmark
    getting `default_log_lr`. A match's real weight in the localization is the
    gap between those two numbers, so the page shows both -- a +4.0 against a
    -4.0 default is the matcher asserting e^8, and that assertion is what moves
    the filter, not the 1.00 next to it.
    """
...
def load_uniqueness(match_dir: Path) -> tuple:
    ...
    m9's response schema *requires* uniqueness_score -- how distinctive the
    observed landmark is on its own -- and we pay tokens for it on every query,
    but m9 keeps it in neither `matches.json` nor `compatibility.json`. It
    survives only in `results.jsonl`, so it is read back here rather than left
    to be a signal that was bought and thrown away.
...
def bearing_residual_deg(rays, east_m: float, north_m: float):
    ...
    for _kf, east, north, world, _course, _kappa in rays:
        to_row = math.degrees(math.atan2(east_m - east,
                                         north_m - north)) % 360.0
        out.append(abs((world - to_row + 180.0) % 360.0 - 180.0))
...
def truth_track(dataset_base: Path):
    ...
    Positions come from `ingest.load_frames`/`fill_enu` and the course from the
    same heading model m11 exports, so the map's idea of where the vessel was
    and which way it faced is the filter's idea. Without `frames_gps.csv` there
    are no timestamps, the model cannot be fitted, and the fallback is a plain
    finite difference -- announced on the page, because a course that came from
    somewhere else is exactly the kind of thing that should not be silent.
    """
    frames = ingest.load_frames(dataset_base)
    anchor_lat, anchor_lon = ingest.fill_enu(frames)
    ...
    if all(t is not None for t in times):
        ...
        course = [model.at(t) for t in times]
        source = "heading model over GPS fixes (the model m11 exports)"
    else:
        course = []
        for i in range(len(east)):
            j = min(i + 1, len(east) - 1)
            k = max(i - 1, 0)
            course.append(math.degrees(math.atan2(east[j] - east[k],
                                                  north[j] - north[k])) % 360.0)
...
def build_map_payload(run_dir: Path, paths, matches: dict, signatures: dict,
                      min_confidence: float, detail: float) -> tuple:
    """Everything the map draws, in ENU metres about the run's anchor.

    Returns (payload, notes). Notes are surfaced on the page -- an unresolved
    landmark id or a truncated expansion changes what the map is showing, so it
    ...
    offset_deg, offset_source = m11_base_export.resolve_mount_offset(
        run_dir, paths.metadata_path, None)
    rows = json.loads((run_dir / "merged" / "measurements.json").read_text())
    measurements, n_fused = m11_base_export.body_frame_measurements(
        rows, offset_deg)

    catalog = harbor_catalog.load_catalog_cached(
        paths.feather, anchor_lat, anchor_lon, keep_hulls=False)
    ...
    for meas in measurements:
        ...
        if kf >= len(east):
            ...
            continue
        world = (course[kf] + meas.bearing_body_deg) % 360.0
    ...
    if unresolved:
        notes.append(f"{len(unresolved)} matched landmark ids are not in "
                     f"{paths.feather.name} and could not be placed "
                     f"(e.g. {sorted(unresolved)[:3]})")

    all_e = list(east) + [t[0] for tk in tracklets.values()
                          for t in tk["targets"]]
    all_n = list(north) + [t[1] for tk in tracklets.values()
                           for t in tk["targets"]]
    ...
    basemap = basemap_mod.build(paths.feather, anchor_lat, anchor_lon,
                               bounds_enu=tuple(bounds), detail=detail)
    ...
    # Inverse of harbor_catalog.enu_from_latlon, as two scale factors: the
    # projection is equirectangular about the anchor, so going back is linear
    # and the page can do it without a projection library.
    per_m = math.degrees(1.0 / harbor_catalog.EARTH_RADIUS_M)
    payload = {
...
// factors come from the payload so this matches harbor_catalog exactly rather
...
def main():
    ...
    parser.add_argument("--detail", type=float, default=2.0,
                        help="basemap vertex budget multiplier; the map is "
                             "zoomable so it is worth more than the 1.0 the "
                             "whole-extent default assumes")
    ...
    # Track pages are named track_<range>_T<id>.html. This used to be a
    # `range_name="full_leg1"` default that neither call site overrode and
    # nothing in this file ever set, so every track link here was dead except
    # on a run whose range happened to be called full_leg1.
    range_name = json.loads(
        next(args.run_dir.glob("tracks_*.json")).read_text())["range"]["name"]
    ...
    if not args.no_map:
        ...
        payload, notes, sig_resid = build_map_payload(
            args.run_dir, paths, matches, signatures, args.min_confidence,
            args.detail)
    ...
    for key, entry in ordered:
        ...
        stats = stats_line(key, entry, lm, uniqueness, log_lr_defaults)
        parts.append(f"<div class='q'><b>observed:</b> "
                     f"<code>{esc(entry['query'])}</code><br>"
                     f"<span class='nomatch'>{stats}</span></div>")
        parts.append(f"<div class='links'>{source_links(key, lm, meta_by_track, range_name)}"
                     "</div>")
    ...
    for key in sorted(miss, key=lambda k: -matches[k]["no_match_confidence"]):
        ...
        parts.append(f"<tr><td>{esc(key)}{onmap}<br>"
                     f"<span class='links'>"
                     f"{source_links(key, lm, meta_by_track, range_name)}</span></td>"
    ...
    if payload is None:
        ...
        parts.append(
            ...
            f"{payload['n_catalog']} catalog rows &middot; basemap "
            f"{esc(str(payload['basemap'].get('source') or 'none'))}</div>")
        for note in notes:
