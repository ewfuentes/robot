...
def wedge_candidates(entries, east_m, north_m, bearing_deg,
                     half_width_deg, max_range_m=None, min_range_m=0.0):
    """Candidates whose angular span overlaps a bearing wedge.

    This is the analogue of LOCI's "OSM landmarks on the panorama's satellite
    tiles": a spatial gate that makes the full catalog affordable. An
    extended candidate qualifies when its span overlaps the wedge at all, so
    an island is not rejected because its centroid sits off-axis.
    """
    out = []
    for entry in entries:
        d_east = entry.east_m - east_m
        d_north = entry.north_m - north_m
        range_m = math.hypot(d_east, d_north)
        if range_m < min_range_m:
            continue
        if max_range_m is not None and range_m > max_range_m:
            continue
        centre, half = bearing_span_from(entry, east_m, north_m)
        if abs(angular_delta_deg(centre, bearing_deg)) <= half_width_deg + half:
            out.append((entry, range_m, centre, half))
