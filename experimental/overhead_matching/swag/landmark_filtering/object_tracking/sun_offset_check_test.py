...
class SolarPositionTest(unittest.TestCase):
    ...
    def test_azimuth_sweeps_east_to_west_through_the_day(self):
        ...
        # Boston mid-morning to evening: monotonically increasing through south.
...
def pano_with_blob(width, height, az_deg, el_deg, value=250, radius=6,
    ...
    x, y = pg.pano_px_from_direction(az_deg, el_deg, width, height)
...
class BlobSearchTest(unittest.TestCase):
    ...
    def test_ignores_a_brighter_blob_outside_the_band(self):
        # This is the sun-glitter case: the water below the horizon is at least
        # as bright as the sun, and a naive argmax lands on it.
    ...
    def test_saturated_flat_topped_blob_is_not_read_off_its_left_edge(self):
        # The regression this module was first wrong about: a saturated sun has
        # a plateau, argmax returns the plateau's left edge, and a window
...
class OffsetAlgebraTest(unittest.TestCase):
    ...
    def test_offset_recovers_a_planted_value(self):
        # Plant a mount offset, synthesise the camera azimuth it would produce,
        # and check the formula inverts it.
