...
class VigorDataset(torch.utils.data.Dataset):
    ...
    def _compute_city_bounding_box(self) -> tuple[float, float, float, float]:
        """Compute bounding box of all panoramas in the dataset.

        Returns:
            (min_lat, max_lat, min_lon, max_lon)
        """
        min_lat = self._panorama_metadata['lat'].min()
        max_lat = self._panorama_metadata['lat'].max()
        min_lon = self._panorama_metadata['lon'].min()
        max_lon = self._panorama_metadata['lon'].max()
        return (min_lat, max_lat, min_lon, max_lon)

    def _compute_path_bounding_box(self, path_indices: list[int]) -> tuple[float, float, float, float]:
        ...
        Returns:
            (min_lat, max_lat, min_lon, max_lon)
        """
        lats = self._panorama_metadata.loc[path_indices, 'lat']
        lons = self._panorama_metadata.loc[path_indices, 'lon']
        return (lats.min(), lats.max(), lons.min(), lons.max())

    def _compute_bbox_coverage_ratio(
        self,
    ...
    def _get_quadrant(self, lat: float, lon: float, city_bbox: tuple[float, float, float, float]) -> int:
    ...
    def _select_distant_goal(
        ...
        """
        current_pos = self._panorama_metadata.loc[current_idx, ['lat', 'lon']].values
        all_positions = self._panorama_metadata[['lat', 'lon']].values
    ...
    def _select_quadrant_goal(
        ...
        """
        current_pos = self._panorama_metadata.loc[current_idx, ['lat', 'lon']].values
        current_quadrant = self._get_quadrant(current_pos[0], current_pos[1], city_bbox)

        all_positions = self._panorama_metadata[['lat', 'lon']].values

        # Find all panoramas in different quadrants
        different_quadrant_mask = np.array([
            self._get_quadrant(pos[0], pos[1], city_bbox) != current_quadrant
    ...
    def _precompute_edge_costs(self) -> list[list[float]]:
        ...
        all_positions = self._panorama_metadata[['lat', 'lon']].values
        edge_costs = []
        for idx in range(len(self._panorama_metadata)):
            neighbors = self._panorama_metadata.loc[idx, 'neighbor_panorama_idxs']
            if len(neighbors) == 0:
                edge_costs.append([])
                continue
    ...
    def _compute_heuristics_to_goal(self, goal_idx: int) -> np.ndarray:
        ...
        """
        goal_pos = self._panorama_metadata.loc[goal_idx, ['lat', 'lon']].values
        all_positions = self._panorama_metadata[['lat', 'lon']].values
    ...
    def generate_goal_directed_path(
        ...
        This method generates paths that spread across the city by:
        ...
        city_bbox = self._compute_city_bounding_box()

        # Pre-compute edge costs (can be cached for repeated calls)
        if not hasattr(self, '_cached_edge_costs'):
            self._cached_edge_costs = self._precompute_edge_costs()
        base_edge_costs = self._cached_edge_costs

        # Get neighbor adjacency list
        neighbors = [list(row['neighbor_panorama_idxs'])
                     for _, row in self._panorama_metadata.iterrows()]
        ...
        def do_random_walk(start_idx: int, target_distance_m: float) -> tuple[list[int], float]:
            ...
            while walk_distance < target_distance_m:
                ...
                # Compute distance
                walk_pos = self._panorama_metadata.loc[walk_idx, ['lat', 'lon']].values
                next_walk_pos = self._panorama_metadata.loc[next_walk_idx, ['lat', 'lon']].values
        ...
        for attempt in range(max_regeneration_attempts):
            ...
            def select_goal(from_idx: int) -> int:
                if quadrant_goals:
                    return self._select_quadrant_goal(from_idx, generator, city_bbox, min_goal_distance_percentile)
                else:
                    return self._select_distant_goal(from_idx, generator, min_goal_distance_percentile)

            # Pick random start
            current_idx = torch.randint(0, len(self._panorama_metadata), (1,), generator=generator).item()
            ...
            while distance_traveled_m < max_length_m:
                ...
                heuristics = self._compute_heuristics_to_goal(goal_idx)
                ...
                for i, next_idx in enumerate(a_star_path):
                    ...
                    prev_pos = self._panorama_metadata.loc[prev_traversed_idx, ['lat', 'lon']].values
                    next_pos = self._panorama_metadata.loc[next_idx, ['lat', 'lon']].values
            ...
            path_bbox = self._compute_path_bounding_box(path_indices)
            coverage = self._compute_bbox_coverage_ratio(path_bbox, city_bbox)

            if coverage >= min_bbox_coverage_ratio:
                # Convert indices to pano_ids
                return [self._panorama_metadata.loc[idx, 'pano_id'] for idx in path_indices]
