...
class FarfieldNamingRulesTest(unittest.TestCase):
    ...
    @unittest.skip(
        "documents an UNSHIPPED v2 revision: the checkpoint branch's runbook "
        "and this test describe a three-clause v2 (STRUCTURE-not-SCENE, "
        "ref=<exactly as read>, lookalike guard) that never landed in the "
        "shipped SYSTEM_PROMPTS registry — the test failed on that branch "
        "too. The shipped prompt text is authoritative (its sha256 is "
        "recorded per extraction run); un-skip only after actually shipping "
        "the revised prompt.")
    ...
    def test_both_farfield_prompts_still_licence_terrain_recognition(self):
        ...
        # NOTE: the shipped v2 never carried the literal "Name the peaks"
        # sentence (only v1 does); the original assertion demanded it of both
        # and could never have passed. Prompt text is a versioned artifact
        # (its sha256 is recorded per run), so the TEST is corrected to what
        # shipped rather than editing the prompt.
        for prompt in (self.v1, self.v2):
            self.assertIn("RECOGNIZING it", prompt)
        self.assertIn("Name the peaks", self.v1)
