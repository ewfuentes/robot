...
class PanoramaSemanticLandmarkExtractor(torch.nn.Module):
    ...
    def __init__(self, config: PanoramaSemanticLandmarkExtractorConfig, semantic_embedding_base_path: Path, trainable_embedder: TrainableSentenceEmbedder | None):
        super().__init__()
        self.config = config
    ...
    def load_files(self):
        ...
        if self.trainable_embedder is None:
            print("[PanoramaSemanticLandmarkExtractor] Loading pre-computed OpenAI embeddings")
            ...
            assert len(self.all_sentences) > 0, "Sentences are required if you are running a trainable embedder"
            print("[PanoramaSemanticLandmarkExtractor] Calculating embeddings using provided trainable embedder")
    ...
    def forward(self, model_input: ModelInput) -> ExtractorOutput:
        ...
        # When not using trainable embedder, use openai_embedding_size
