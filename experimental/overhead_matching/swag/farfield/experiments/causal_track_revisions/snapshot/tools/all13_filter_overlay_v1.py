"""All13 endpoint adapter: last timestamp allowed; no EOF flush or fabricated releases."""
...
def prepare_releases(evidence, timestamps, end):
    if not 0 <= end < len(timestamps):
        raise ValueError('Output endpoint must be an existing timestamp')
