...
def _apply_log_likelihood(message, log_factor):
    """Include the incoming message before rescaling co-released factors."""
...
def main():
    ...
    parser.add_argument("--factor_cache_dir", default=None)
    parser.add_argument("--adaptive_tables", default=None,
                        help="Experimental: use alternative track-local tables while spatial belief is diffuse")
    parser.add_argument("--adaptive_extent_observations", default=None,
                        help="Experimental: ready track extent observations for point/footprint agreement")
    parser.add_argument("--adaptive_lock_radius_m", type=float, default=500.0)
    parser.add_argument("--adaptive_lock_mass", type=float, default=0.9)
    parser.add_argument("--resume_prior", default=None,
                        help="DIAGNOSTIC: saved pre-release prior; starts a conditional window")
    parser.add_argument("--window_end", type=int, default=-1,
                        help="DIAGNOSTIC: final included keyframe; no synthetic EOF flush")
    parser.add_argument("--trace_dir", default=None,
                        help="Offline diagnostic tensor output; never affects filtering")
    ...
    if args.adaptive_tables and (args.smoother != "none" or not args.track_joint
        ...
        parser.error("adaptive identity requires natural joint filtering without smoothing")
    if args.adaptive_extent_observations and (args.smoother != "none" or not args.track_joint
            or args.availability != "natural" or args.extent_sigma or args.joint_cap is not None):
        parser.error("adaptive geometry requires uncapped natural joint point filtering without smoothing")
    if (not math.isfinite(args.adaptive_lock_radius_m) or args.adaptive_lock_radius_m <= 0
            or not math.isfinite(args.adaptive_lock_mass) or not 0 < args.adaptive_lock_mass <= 1):
        parser.error("adaptive lock requires a positive finite radius and mass in (0,1]")
    if (args.resume_prior or args.window_end >= 0) and (
            args.smoother != "none" or not args.track_joint or args.availability != "natural"):
        parser.error("diagnostic windows require natural joint filtering without smoothing")
    trace_keyframes = {int(k) for k in args.trace_keyframes.split(",") if k}
    if trace_keyframes and (not args.trace_dir or not args.track_joint):
        parser.error("tracing requires --trace_dir and joint mode")
    ...
    weight_cache = {}
    catalog_extent = None
    if args.extent_sigma or args.adaptive_extent_observations:
    ...
    extent_observations = {}
    geometry_decisions = []
    if args.adaptive_tables or args.adaptive_extent_observations:
        from experimental.overhead_matching.swag.farfield.localization import adaptive_identity
    if args.adaptive_extent_observations:
        from experimental.overhead_matching.swag.farfield.localization import adaptive_geometry
        extent_observations = adaptive_geometry.load_observations(
            Path(args.adaptive_extent_observations), data.tables)
    if args.adaptive_tables:
        from experimental.overhead_matching.swag.farfield.localization import adaptive_identity
        adaptive_alternatives = adaptive_identity.load_alternatives(
            Path(args.adaptive_tables), data.tables)
        print(f"Experimental adaptive identity: {len(adaptive_alternatives)} alternatives")
    ...
    def candidate_set(tracklet_id, use_extent=None):
        use_extent = bool(args.extent_sigma) if use_extent is None else use_extent
        cache_key = (tracklet_id, use_extent)
        if cache_key not in weight_cache:
            ...
            weight_cache[cache_key] = (
                ...
                tail_mass,
                catalog_extent[idx_tensor] if use_extent and catalog_extent is not None
                else None)
    ...
    def keyframe_log_likelihood(keyframe, grid_belief):
        """Log product: preserve dynamic range until the incoming message is included."""
        factor = None
        heading_factor = heading_meas.get(keyframe)
        if heading_factor is not None:
            factor = torch.log(heading_factor.expand_as(grid_belief.belief))
        if args.track_joint:
            releases_at = {}
    ...
    def smoothing_pass():
        ...
        def backward_step(beta, keyframe):
            if keyframe in factor_window:
                ...
                factor = keyframe_log_likelihood(keyframe, planner)
            incoming = _apply_log_likelihood(beta, factor)
            return _normalized(
        ...
        for keyframe in range(n_keyframes):
            ...
            factor = keyframe_log_likelihood(keyframe, planner)
            message = _apply_log_likelihood(message, factor)
        ...
        for seg_start in segment_starts:
            ...
            for keyframe in range(seg_start + 1, seg_end):
                ...
                factor = keyframe_log_likelihood(keyframe, planner)
                current = _apply_log_likelihood(current, factor)
                alphas.append(current)
    ...
    if args.availability == "natural":
        ...
        schedule_schema = json.loads(Path(args.release_schedule).read_text())["schema"]
        release_policy = ("cached_prompt_dependency_lower_bound"
                          if schedule_schema == release_schedule_lib.DEPENDENCY_SCHEMA
                          else "natural_track_close_with_eof_flush")
        release_counts = {}
        release_tracklets = {}
        for release in releases:
        ...
        if args.factor_cache_dir:
            ...
            excluded = {"factor_cache_dir", "out", "trace_dir", "trace_keyframes",
                        "resume_prior", "window_end", "tables_override", "joint_cap",
                        "smoother", "smooth_lag", "smooth_lags", "online_seconds",
                        "checkpoint_keyframes", "top_modes", "likelihood_cache_gb",
                        "profile_releases", "release_top_modes", "adaptive_tables",
                        "adaptive_lock_radius_m", "adaptive_lock_mass", "adaptive_extent_observations"}
            source_hashes = {p.name: hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in Path(__file__).parent.glob("*.py")}
            disk_factors = FactorCache(args.factor_cache_dir, {
                "config": {k:v for k,v in vars(args).items() if k not in excluded},
                "inputs": data.artifact_ref.to_dict(), "source": source_hashes,
                "identity_override": identity_override, "grid_box": box,
                "schedule_sha256": hashlib.sha256(Path(args.release_schedule).read_bytes()).hexdigest()})

        def joint_release_likelihood(release, keyframe, grid_belief=None, use_extent=None):
            grid_belief = belief if grid_belief is None else grid_belief
            use_extent = bool(args.extent_sigma) if use_extent is None else use_extent
            def compute():
                def uncapped():
                    return _joint_release_likelihood(release, keyframe, grid_belief, use_extent)
                identity = {"track": release.tracklet_id, "keyframe": keyframe,
                            "extent_sigma": int(use_extent), "table": msgspec.json.encode(data.tables[release.tracklet_id]).decode()}
                value = (disk_factors.get(identity, args.device, uncapped)
                         if disk_factors is not None else uncapped())
                if args.joint_cap is not None:
                    tail_mass = candidate_set(release.tracklet_id)[3]
                    limit = value.new_tensor(args.joint_cap)
                    limit = args.pi0 + (1.0-args.pi0)*(limit+tail_mass)
                    value = torch.clamp(value, max=limit)
                return value
            return likelihood_cache.get(("joint", release.tracklet_id, keyframe, use_extent), compute)

        def _joint_release_likelihood(release, keyframe, grid_belief, use_extent=None):
            (candidate_east, candidate_north, candidate_weight,
             tail_mass, candidate_extent) = candidate_set(
                release.tracklet_id, use_extent)
        ...
        if args.track_joint:
            ...
            update_timings = []
            start_keyframe = 0
            end_keyframe = n_keyframes-1 if args.window_end < 0 else args.window_end
            window_run = bool(args.resume_prior or args.window_end >= 0)
            if args.resume_prior:
                checkpoint = torch.load(args.resume_prior, map_location="cpu", weights_only=True)
                start_keyframe = checkpoint["keyframe"]
                if checkpoint.get("inputs") != data.artifact_ref.to_dict():
                    raise ValueError("diagnostic prior belongs to different localization inputs")
                if checkpoint.get("odometry_seed") != args.odometry_seed:
                    raise ValueError("diagnostic prior has a different odometry seed")
                if checkpoint["prior"].shape != belief.belief.shape:
                    raise ValueError("diagnostic prior grid shape mismatch")
                if not (torch.equal(checkpoint["cell_east"], belief.cell_east.cpu())
                        and torch.equal(checkpoint["cell_north"], belief.cell_north.cpu())):
                    raise ValueError("diagnostic prior grid coordinates mismatch")
                belief.belief = checkpoint["prior"].to(args.device)
                # A belief alone is insufficient: sub-cell motion and uncertainty
                # accumulators carry history. Reconstruct only that cheap planner
                # state; never reapply prefix motion to the saved belief.
                for step in range(1, start_keyframe+1):
                    belief.plan_motion(odometry[step], args.yaw_sigma_scale,
                                       heading_rw_rad, args.diffusion_m)
            if window_run and start_keyframe == end_keyframe:
                raise ValueError("diagnostic windows require at least two keyframes")
            if not 0 <= start_keyframe <= end_keyframe < n_keyframes:
                raise ValueError("invalid diagnostic window bounds")
            for keyframe in range(start_keyframe, end_keyframe+1):
                update_started = time.perf_counter()
                resumed_frame = bool(args.resume_prior and keyframe == start_keyframe)
                if keyframe > 0 and not resumed_frame:
                    belief.motion(
                        odometry[keyframe], args.yaw_sigma_scale,
                        heading_rw_rad, args.diffusion_m)
                heading_factor = heading_meas.get(keyframe)
                if heading_factor is not None and not resumed_frame:
                    belief.belief = _normalized(belief.belief * heading_factor)
                trace_path = None
                if keyframe in trace_keyframes:
                    trace_path = Path(args.trace_dir) / f"f{keyframe:04d}"
                    trace_path.mkdir(parents=True, exist_ok=True)
                    truth_pose = truth_by_kf[keyframe]
                    torch.save({
                        "prior": belief.belief.detach().cpu(),
                        "cell_east": belief.cell_east.detach().cpu(),
                        "cell_north": belief.cell_north.detach().cpu(),
                        "truth_east": float(truth_pose.east_m),
                        "truth_north": float(truth_pose.north_m),
                        "keyframe": keyframe,
                ...
                for release in releases_at.get(keyframe, ()):
                    if release.tracklet_id in adaptive_alternatives:
                        mode_mass = adaptive_identity.spatial_mode_mass(
                            belief.belief, belief.cell_east, belief.cell_north,
                            args.adaptive_lock_radius_m)
                        expanded = mode_mass < args.adaptive_lock_mass
                        if expanded:
                            # Atomic tracks release once. Select before any candidate
                            # or likelihood lookup; the disk key contains this table.
                            data.tables[release.tracklet_id] = adaptive_alternatives[release.tracklet_id]
                            weight_cache.pop((release.tracklet_id, False), None)
                            weight_cache.pop((release.tracklet_id, True), None)
                        adaptive_decisions.append({
                            "keyframe": keyframe, "tracklet_id": release.tracklet_id,
                            "pre_update_spatial_mode_mass": mode_mass,
                            "choice": "expanded" if expanded else "narrow"})
                    ...
                    if args.adaptive_extent_observations:
                        mode_mass = adaptive_identity.spatial_mode_mass(
                            belief.belief, belief.cell_east, belief.cell_north,
                            args.adaptive_lock_radius_m)
                        likelihood, geometry_decision = adaptive_geometry.choose_factor(
                            belief.belief, likelihood,
                            lambda: joint_release_likelihood(release, keyframe, use_extent=True),
                            belief.cell_east, belief.cell_north, _normalized,
                            large_extended=adaptive_geometry.large_at_release(
                                extent_observations, release.tracklet_id, keyframe),
                            mode_mass=mode_mass, radius_m=args.adaptive_lock_radius_m,
            ...
            filtered_summary = _summary(
                filtered_series, filtered_map_error,
                {i: truth_by_kf[start_keyframe+i] for i in range(end_keyframe-start_keyframe+1)},
                end_keyframe-start_keyframe+1)
            ...
            if args.out:
                payload = {
                    "schema": "farfield_grid_window/v1" if window_run else "farfield_causal_grid/v1",
                    "diagnostic_window": ({"start": start_keyframe, "end": end_keyframe,
                        "conditional_on_saved_prior": bool(args.resume_prior)} if window_run else None),
                    "adaptive_geometry": ({"policy": "map_agreement_gate_v1",
                    ...
                    "adaptive_identity": ({"policy": "expand_while_spatially_diffuse_v1",
                        "radius_m": args.adaptive_lock_radius_m,
                        "lock_mass": args.adaptive_lock_mass,
                        "decisions": adaptive_decisions} if args.adaptive_tables else None),
                    "factor_cache": ({"hits":disk_factors.hits,"misses":disk_factors.misses}
                        if disk_factors is not None else None),
                    ...
                    "online_map_state_by_keyframe": {
                        ...
                        "keyframe_order": "keyframe_idx_equals_list_index_plus_start",
                        "start_keyframe": start_keyframe,
                        "states": online_map_states,
                    },
                    "filtered_final_top_modes": {
                        "source": "online_current_posterior_at_last_processed_keyframe",
                        "reference_keyframe_idx": end_keyframe,
                        "pose_frame": (
