...
def _rings(geometry, frame: geo.RegionFrame, simplify_m: float) -> list:
...
def build(feather_path: Path | None, anchor_lat_deg: float,
    ...
    try:
        # The one sanctioned feather reader (farfield/catalog/schema.py); its
        # geo dependencies (geopandas/pyarrow) are heavier than this module's,
        # so their absence degrades to an empty basemap rather than an error.
        from experimental.overhead_matching.swag.farfield.catalog import (
            schema as landmark_schema,
        )
    except ImportError as exc:  # pragma: no cover - dependency shape
        return Basemap([], str(feather_path),
                       (f"geometry dependencies unavailable: {exc}",))

    frame = geo.RegionFrame(anchor_lat_deg, anchor_lon_deg)
    notes: list[str] = []
    try:
        frame_data = landmark_schema.read_frame(feather_path)
