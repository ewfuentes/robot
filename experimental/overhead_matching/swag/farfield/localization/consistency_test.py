...
class PositionConsistencyTest(unittest.TestCase):
    def test_nees_within_chi_squared_bounds(self):
        ...
        final = truth[-1]
        nees = np.array([metrics.position_nees(b, final.east_m, final.north_m)
                         for b in beliefs])
    ...
    def test_error_and_reported_sigma_agree(self):
        ...
        final = truth[-1]
        errors = np.array([
            math.hypot(metrics.mean_pose(b)[0] - final.east_m,
                       metrics.mean_pose(b)[1] - final.north_m) for b in beliefs])
        sigmas = np.array([metrics.position_std_m(b) for b in beliefs])
    ...
    def test_unregularized_resampling_is_overconfident(self):
        ...
        for regularization in (0.0, 1.0):
            ...
            nees_by_setting[regularization] = float(np.mean([
                metrics.position_nees(b, final_truth.east_m, final_truth.north_m)
                for b in beliefs]))
...
class HeadingConsistencyTest(unittest.TestCase):
    def test_reported_heading_sigma_covers_error(self):
        ...
        likelihood."""
        config = scenario.harbor_loop(max_visible_range_m=10000.0, keyframe_period_s=_PERIOD_S)
...
class BearingsAnchorHeadingTest(unittest.TestCase):
    def test_heading_is_unobservable_without_bearings(self):
        ...
        self.assertGreater(metrics.heading_std_deg(without.final_belief), 60.0,
        ...
        with_bearings = pf.run_filter(config, data.catalog, data.odometry,
                                      data.measurements, data.tables)
        self.assertLess(metrics.heading_std_deg(with_bearings.final_belief), 5.0,
