...
def epoch_term(log_prod, east, north, cand_east, cand_north, p):
    ...
    quant = p[13] * (p[7] + (p[8] / safe_distance) ** 2)
    ...
    term = term + p[14] * torch.log((1.0 - p[12]) * soft + p[12] + 1e-30)
    ...
    for h in belief.bin_rad:
        parameters.append([
            [df * math.sin(h) - dl * math.cos(h),
             ...
             float(quantization_comp), float(rm is not None)]
