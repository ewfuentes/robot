...
def _landmark_positions(data):
    frame = geo.RegionFrame(data.manifest.anchor_lat_deg,
...
def _draw_strip(data, axes):
    ...
    if data.truth:
        ...
        # against exactly those keyframes rather than the full axis.
        truth_kfs = {t.keyframe_idx for t in data.truth}
        error_kfs = [h.keyframe_idx for h in data.health
