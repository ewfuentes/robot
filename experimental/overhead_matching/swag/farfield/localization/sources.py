...
COMPATIBILITY NOTE: the `merged/` artifacts this reads were written by the
checkpoint branch's merge stage, which the reorganization ELIMINATED (REORG.md,
"The merge stage is eliminated") — new tracking runs will not produce them.
This loader exists so the viewer can still open the existing corpus of runs;
on a run without them every field degrades to a note, exactly as designed.
When the new tracklets-based export grows its own payload sidecar, teach this
loader that layout and delete the merged/ paths.
"""
