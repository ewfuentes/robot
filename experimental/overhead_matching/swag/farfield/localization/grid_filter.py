...
Revived 2026-09-06 (likelihood-model branch): identity weights follow
FilterConfig.identity_weights (global softmax / in-cap renormalized /
flat), each measurement's range cap multiplies its landmark terms, and the
summary carries the distance-normalized mass the PF reports.
...
Run:
  bazel run //experimental/overhead_matching/swag/farfield/localization:grid_filter -- \
    ...
    [--like_run <run dir>]   # else: catalog extent + --margin_m, PF parity
"""
...
class GridBelief:
    ...
    def track_likelihood(self, epochs, cand_east: torch.Tensor,
                         ...
                         chunk: int = 256, range_max_m=None,
                         range_softness: float = 0.25,
                         visible: bool = False) -> torch.Tensor:
        ...
        `range_max_m` is the track's tightest range cap g(r)
        (filter.range_cap_log_term), applied per candidate at the cell.
        `visible` renormalizes the identity prior per cell over what the cap
        admits, p(j|cell) = w_j g_j / (tail + sum_k w_k g_k) — the PF's
        visible_* identity weights; the tail is treated as always admissible.
        """
        ...
        like = torch.zeros((n_heading, n_cells), dtype=torch.float32,
                           device=self.device)   # sum_j w_j g_j prod_i vM
        zsum = torch.full((n_cells,), float(tail_mass), dtype=torch.float32,
                          device=self.device)    # tail + sum_j w_j g_j
        ...
        for start in range(0, cand_east.shape[0], chunk):
            ...
            gate = cand_weight[sl][None, :].expand_as(rng)
            if range_max_m is not None:
                excess = torch.clamp(rng - range_max_m, min=0.0)
            ...
            for b in range(n_heading):
                ...
                like[b] += (torch.exp(exponent) * gate).sum(dim=1)
        landmark = like + tail_mass          # tail is a uniform bearing term
        if visible:
            landmark = landmark / zsum[None, :]
        like = pi0 + (1.0 - pi0) * landmark
...
def main():
    ...
    parser.add_argument("--margin_m", type=float, default=1000.0,
                        help="box = catalog extent + margin when no "
                             "--like_run (the PF's localization.margin_m)")
    ...
    if args.like_run:
        ...
        region = export_ingest.region_box(data, args.margin_m)
    ...
    for keyframe in range(data.n_keyframes):
        ...
        for meas in by_keyframe.get(keyframe, ()):
            ...
            def epoch_view(record):
                z_i, kappa_i, yaw_i, var_i, _cap = record
            ...
            recent = history[-(args.max_track_epochs - 1):]
            prev = [epoch_view(record) for record in recent]
            new = prev + [(z_rad, 1.0 / kappa_z)]
            caps = [meas.range_max_m] + [record[4] for record in recent]
            caps = [c for c in caps if c is not None] if args.range_cap else []
            cap_new = min(caps) if caps else None
            caps_prev = [c for c in caps[1:]] if args.range_cap else []
            ...
            if prev:
                like = like / belief.track_likelihood(
                    ...
                    args.pi0, tail_mass, range_max_m=cap_prev, **kw)
            belief.belief *= like
    ...
    summary = {}
    # Distance along the truth trajectory (the PF's distance-normalized
    # metric, metrics.distance_normalized_position_mass): trapezoid over
    # travelled distance / total distance.
    dist = np.zeros(data.n_keyframes)
    for k in range(1, data.n_keyframes):
        a, b = truth_by_kf[k - 1], truth_by_kf[k]
        dist[k] = dist[k - 1] + math.hypot(b.east_m - a.east_m,
                                           b.north_m - a.north_m)
    for radius in radii:
        ...
        summary[f"dn_mass_{radius:g}"] = float(
            np.sum(0.5 * (values[:-1] + values[1:]) * np.diff(dist))
            / max(dist[-1], 1e-9))
    best = int(torch.argmax(belief.position_marginal()))
    pose = truth_by_kf[data.n_keyframes - 1]
    summary["final_map_error_m"] = math.hypot(
        east_centers[best % grid.n_east] - pose.east_m,
        north_centers[best // grid.n_east] - pose.north_m)
    print("mass summary:",
