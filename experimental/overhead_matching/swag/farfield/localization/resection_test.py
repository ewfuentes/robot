...
def _bearings_from(pose, landmarks):
    ...
    return [float(geo.wrap_rad(
        geo.compass_bearing_rad(le - east, ln - north) - heading))
...
class InscribedArcTest(unittest.TestCase):
    def test_every_arc_point_reproduces_the_angle(self):
        ...
        for gamma_deg in (20.0, 55.0, 90.0, 130.0):
            ...
            for arc in arcs:
                ...
                observed = np.abs(geo.wrap_rad(
                    geo.compass_bearing_rad(east_b - east, north_b - north)
                    - geo.compass_bearing_rad(east_a - east,
                                                  north_a - north)))
    ...
    def test_selected_arc_reproduces_the_signed_angle(self):
        ...
        observed = geo.wrap_rad(
            geo.compass_bearing_rad(landmarks[1][0] - east,
                                        landmarks[1][1] - north)
            - geo.compass_bearing_rad(landmarks[0][0] - east,
                                          landmarks[0][1] - north))
    ...
    def test_round_trip_recovers_pose(self):
        ...
        for _ in range(200):
            ...
            self.assertAlmostEqual(
                float(geo.wrap_rad(best.heading_rad - pose[2])), 0.0,
                delta=1e-6)
...
class HeadingRecoveryTest(unittest.TestCase):
    ...
    def test_round_trip_against_arbitrary_poses(self):
        ...
        for _ in range(50):
            ...
            bearing = float(geo.wrap_rad(
                geo.compass_bearing_rad(lm_east - east, lm_north - north)
                - heading))
            recovered = resection.heading_from_bearing(east, north, lm_east,
                                                       lm_north, bearing)
            self.assertAlmostEqual(
                float(geo.wrap_rad(recovered - heading)), 0.0, places=12)
