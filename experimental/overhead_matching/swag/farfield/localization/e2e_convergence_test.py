...
class LocalConvergenceTest(unittest.TestCase):
    def test_converges_and_stays(self):
        cfg = scenario.harbor_loop(max_visible_range_m=10000.0, keyframe_period_s=_PERIOD_S)
...
class GlobalInitConvergenceTest(unittest.TestCase):
    def test_uniform_box_converges(self):
        ...
        cfg = scenario.harbor_loop(
            max_visible_range_m=10000.0, keyframe_period_s=_PERIOD_S, epoch_length_keyframes=3,
        ...
        mass = metrics.mass_within_radius(history.final_belief, final_truth.east_m,
                                     final_truth.north_m, 250.0)
        self.assertGreater(mass, 0.5,
                           f"only {mass:.0%} of the posterior within 250 m "
                           f"of truth")
        map_errors = metrics.map_position_errors_m(history.health, data.truth)
...
class OdometryOnlyTest(unittest.TestCase):
    def test_dead_reckoning_fans_out_honestly(self):
        ...
        cfg = scenario.straight_leg(max_visible_range_m=10000.0, keyframe_period_s=_PERIOD_S)
        data = scenario.generate(cfg)
        filter_config = structs.FilterConfig(
        ...
        final = data.truth[-1]
        nees = metrics.position_nees(history.final_belief, final.east_m,
                                final.north_m)
        self.assertLess(nees, 8.0,
                        f"odometry-only belief is dishonest about its "
                        f"ignorance: NEES {nees:.1f}")
        self.assertGreater(metrics.heading_std_deg(history.final_belief), 60.0,
    ...
    def _clutter_runs(self, pi0, matcher_recall=None):
        ...
        for seed in self._SEEDS:
            ...
            histories.append(pf.run_filter(filter_config, data.catalog,
                                           ...
                                           data.tables))
        errors = [float(metrics.position_errors_m(h.health, data.truth)[-1])
                  for h in histories]
...
class MultimodalityPreservationTest(unittest.TestCase):
    ...
    def test_symmetric_landmarks_keep_both_modes(self):
        cfg = scenario.symmetric_pair(max_visible_range_m=10000.0, keyframe_period_s=_PERIOD_S)
        data = scenario.generate(cfg)
        ...
        truth = data.truth[-1]
        self.assertGreater(
            metrics.mass_within_radius(history.final_belief, truth.east_m,
                                  truth.north_m, 400.0),
    ...
    def test_estimates_agree_across_counts(self):
        ...
        for n_particles in (10000, 40000):
            ...
            history = pf.run_filter(filter_config, data.catalog,
                                    data.odometry, data.measurements,
                                    data.tables)
            estimates.append(metrics.mean_pose(history.final_belief))
