"""`forensics` — the viewer's primitives on the command line (design doc §7.5).
...
forensics and CI". Same modules, same answers, no browser:
  ...
  forensics check       is this run faithfully replayable, and what is missing
  forensics attribute   build the Tier-3 cache; print a mode's waterfall
  forensics replay      run a counterfactual and write it as a run directory
  forensics tracklet    one tracklet's dossier, including the truth triage
  forensics events      the §7.3 event index
  forensics triage      the per-tracklet culpability table for a whole run

(This CLI was called `runlog`, one underscore away from the `run_log` I/O
...
--------
  ...
  forensics triage --run_dir RUN --sort worst

  # Build attribution once, then ask about a mode.
  forensics attribute --run_dir RUN
  forensics attribute --run_dir RUN --mode 0 --from_kf 290 --to_kf 310

  # Was tracklet LT267 actually the culprit?
  forensics replay --run_dir RUN --without_tracklet LT267

  # Would the run have converged if the matcher had got LT267 right?
  forensics replay --run_dir RUN --force_landmark LT267=enc:0226025A1C7615CA
"""
...
def cmd_attribute(args) -> int:
    ...
    if cache is None:
        ...
        cache, result = attribution_mod.compute(
            args.run_dir, verify=not args.allow_divergence)
...
def cmd_replay(args) -> int:
    ...
    if edits.is_empty:
        print("no edits given: this would just re-run the baseline. Use "
              "`forensics check --replay` for that.", file=sys.stderr)
        return 2
    ...
    output_dir = args.output_dir or replay_mod.default_counterfactual_dir(
        args.run_dir, edits)
    replay_mod.write_counterfactual(output_dir, args.run_dir, result)

    baseline, _ = _load(args.run_dir)
    ghost = run_io.read_run(output_dir)
    base_errors = metrics.map_position_errors_m(baseline.health,
                                                baseline.truth)
    ghost_errors = metrics.map_position_errors_m(ghost.health, ghost.truth)
    print()
...
def cmd_tracklet(args) -> int:
    ...
    if table is None:
        ...
        endorsed_ids = forensics.endorsed_entries(table)
        endorsed = [e for e in table.entries if e.landmark_id in endorsed_ids]
    ...
    if cache is not None:
        ...
        print("  filter: no attribution cache (run `forensics attribute`)")
...
def cmd_triage(args) -> int:
    ...
    if not nats:
        print("\n(no attribution cache, so the nats column is empty; run "
              "`forensics attribute` to fill it)")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        prog="forensics", description=__doc__,
