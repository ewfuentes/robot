...
class ResidualTest(unittest.TestCase):
    ...
    def test_condition_gate_drops_glancing_geometry(self):
        # A landmark 2 km ahead of a 400 m run is seen along almost one
        # bearing, so its rays intersect at a glancing angle: condition
        # ~5.5e4. The residual is still ~0, which is the whole point of
        # gating on both -- the position is uncertain by kilometres along the
        # line of sight.
...
class TriangulateTest(unittest.TestCase):
    ...
    def test_solution_behind_the_observers_is_refused(self):
        # The least-squares solve intersects LINES; rays pointing away from
        # their intersection still "intersect" behind the observers, and that
        # solution is meaningless.
        point = (50.0, -100.0)
        rays = []
        for east, north in ((0.0, 0.0), (100.0, 0.0)):
            toward = geo.compass_bearing_deg(point[0] - east, point[1] - north)
            rays.append((east, north, (toward + 180.0) % 360.0))
        self.assertIsNone(mos.triangulate(rays))

    def test_single_ray_is_under_determined(self):
        self.assertIsNone(mos.triangulate([(0.0, 0.0, 45.0)]))
...
class SupportGateTest(unittest.TestCase):
    ...
    def test_excludes_the_leg1_style_low_support_spike(self):
        # The real failure: a 5-tracklet candidate edging out a 37-tracklet
        # one.
...
class UnderSupportTest(unittest.TestCase):
    ...
    def test_one_tracklet_is_refused_however_clean_the_curve(self):
        # The observed shape: a single tracklet traces a perfectly smooth
        # bowl, and every relative gate passes it.
...
class ArcGateTest(unittest.TestCase):
    ...
    def test_arc_is_the_smallest_containing_span(self):
        self.assertAlmostEqual(
            mos.arc_deg(self.observations([10.0, 30.0, 50.0])), 40.0,
            places=6)

    def test_arc_wraps_around_north(self):
        # 350, 0, 10 spans 20 deg, not 350.
        self.assertAlmostEqual(
            mos.arc_deg(self.observations([350.0, 0.0, 10.0])), 20.0,
            places=6)
    ...
    def test_course_variation_counts_toward_the_arc(self):
        # A tracklet held at a constant camera bearing while the vehicle
        # turns still sweeps the world bearing, and that is what
        # triangulation uses.
