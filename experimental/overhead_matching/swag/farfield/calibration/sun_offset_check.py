...
intersect, which fixes the angle only up to whatever the poses and the
heading model get wrong together. Bow calibration measures the bow, which is
not the direction of travel. The operator's prior is a look at one frame.
...
with **no map, no tracks, and no tracklet geometry** involved. That makes it
the only check here that can catch an error the sweep and the poses share --
most importantly a 180 deg convention slip, which a relative method
reproduces happily.

Why this is not circular: the offset is a *yaw* rotation, so it cannot change
the sun's **elevation**. The ephemeris elevation is therefore free to gate
the search -- it says which bright blob is the sun rather than sun-glitter on
the water, cloud, or a lamp -- while the ephemeris **azimuth** is held back
as the quantity being compared. Blobs are accepted on elevation and scored on
azimuth.
...
- **The vehicle's own sunlit structure.** This is the one that bites hardest.
  ...
  neither brightness nor the elevation gate rejects it -- and a blob that
  holds still in the *camera* frame while the vehicle turns cannot be the
  sun. Two defences, both cheap: a **rig mask** built from the temporal
  median over the sampled frames (structure bolted to the vehicle is the only
  thing bright in every frame, since the sun sweeps the camera frame as the
  vehicle turns), and a **fixed-object model test** that fits `az_camera =
  const` alongside the sun model and reports which explains the frames
  better.
- **Overcast.** A blown-out sky puts the brightest blob on zenith cloud. The
  elevation gate rejects most of those, and what survives scatters; the
  reported circular concentration R collapses.
- **Pitch and roll.** A pitching boat tilts the sun's apparent azimuth by a
  few degrees. It averages out over frames but sets the floor on precision;
  treat a spread of a few degrees as normal and do not read the mean past
  ~2 deg.
- **Sun behind the mast.** Occluded frames simply find no blob in the band.

Read `R` first. Above ~0.95 the frames agree and the mean is a real
measurement; below ~0.8 the tool found weather, not the sun, and says nothing
either way.

Tuning history: see docs/farfield (previously inline here).

Writes `<run_dir>/sun_offset_check.json` -- a sidecar in the run directory,
ALWAYS (previously nothing was written unless --json_out was passed). It
NEVER touches the dataset's `pipeline_metadata.json`: datasets are frozen
(REORG.md rule 7). A separate, explicit "publish to dataset metadata" tool
(with the accuracy-validated guard and checksum regeneration) will land
later; until then, consumers read the sidecar.

    bazel run //experimental/overhead_matching/swag/farfield/calibration:sun_offset_check -- \\
        --run_dir <runs>/<run> --n_frames ... --min_speed_mps ... \\
        --elevation_tolerance_deg ... --work_width ...
"""
...
# overfitting: the gate cannot see the quantity being estimated
# (--scan_tolerance prints that table for a run). Widen this only for a
# platform with genuine pitch, and expect the ghost back when you do.
# Tuning history: see docs/farfield (previously inline here).
...
SIDECAR_NAME = "sun_offset_check.json"
...
def fixed_object_concentration(rows):
    ...
    `R_sun` is the agreement of the recovered offsets, which is the sun
    model. `R_fixed` is the agreement of the raw camera azimuths, which is
    the model "some bright thing is bolted to the camera". Whichever is
    larger names what was actually being tracked, and the comparison is only
    meaningful when the course varied -- otherwise the two models are the
    same model.
    """
...
def assess(concentration: float, r_fixed: float, course_r: float):
    """(verdict, detail, usable) from the run's circular statistics.

    `concentration` is R over the recovered offsets (the sun model),
    `r_fixed` is R over the raw camera azimuths (the fixed-object model), and
    `course_r` is R over the courses (near 1 means the course barely varied,
    so the two models are indistinguishable).

    `usable` is the publishing gate, and it is verdict-gated ON PURPOSE: it
    is True only for the sun-verified verdict (AGREEING) with the
    concentration bar met. Gating on concentration alone was a real bug: a
    FIXED-OBJECT verdict is an abstention -- the bright blob tracked the
    vehicle's own structure, so its "offset" is a property of the rig, not
    the mount -- yet its per-frame offsets can still agree with each other
    (R above the bar), and the old code would publish an accuracy-validated
    number from exactly that failure mode.
    """
    if r_fixed > concentration and course_r < 0.98:
        ...
        detail = (f"the blobs hold still in the camera frame (R_fixed="
                  ...
                  f"(R_sun={concentration:.3f}), while the course varied. "
                  f"That is the vehicle's own structure, not the sun. This "
                  f"check abstains")
        ...
        verdict = "SCATTERED"
        detail = (f"R={concentration:.3f} -- the blobs are not the sun "
                  f"(cloud, glare, or occlusion). This check abstains")
    usable = verdict == "AGREEING" and concentration >= R_TRUSTWORTHY
    return verdict, detail, usable


def write_sidecar(run_dir: Path, record: dict) -> Path:
    """Write the check's result JSON into the run dir -- always.

    Written unconditionally, whatever the verdict: an abstention on disk is
    the record that stops the next person from re-deriving it. The sidecar is
    the ONLY thing this tool writes -- it never touches the dataset's
    pipeline_metadata.json (datasets are frozen, REORG.md rule 7); publishing
    a usable result into dataset metadata is a separate, explicit tool. The
    convention/frame/provenance fields are stamped here so every sidecar
    carries them by construction.
    """
    record = dict(record)
...
def main():
    ...
    parser.add_argument("--run_dir", type=Path, required=True,
                        help="Tracking run dir; infers the dataset and "
                             "receives the sun_offset_check.json sidecar")
    ...
    parser.add_argument("--n_frames", type=int, required=True,
                        help="How many frames to sample across the leg "
                             "(previously 40 on the harbor datasets)")
    parser.add_argument("--min_speed_mps", type=float, required=True,
                        help="Course is meaningless when nearly stopped, and "
                             "the offset is measured against course "
                             "(previously 0.8 on the harbor datasets)")
    parser.add_argument("--elevation_tolerance_deg", type=float,
                        required=True,
                        help="Half-width of the elevation band searched "
                             "around the ephemeris elevation; see the "
                             "module comment on why it wants to be tight "
                             "(previously 3.0 on the harbor datasets)")
    parser.add_argument("--work_width", type=int, required=True,
                        help="Panoramas are searched at this width; 1440 is "
                             "0.25 deg/px, finer than the blob centroid "
                             "(previously 1440 on the harbor datasets)")
    parser.add_argument("--no_mask_rig", dest="mask_rig",
                        action="store_false",
                        help="Disable masking structure that is bright in "
                             "the temporal median, i.e. bolted to the "
                             "camera (masking is on unless disabled)")
    parser.add_argument("--scan_tolerance", action="store_true",
                        help="Report the whole tolerance-vs-R table instead "
                             "of one verdict. That table is the evidence: R "
                             "must rise as the band tightens if the blob is "
                             "the sun")
    ...
    # The camera azimuth below is a pano-column azimuth; on a north-aligned
    # dataset the subtraction would double-count heading with every number
    # still well-formed.
    ...
    # Speed from consecutive keyframe positions, the same way the heading
    # model sees it -- `Frame` carries no speed field, and the GPS CSV's own
    # column is not what the course was derived from.
    fast = []
    ...
    # Load the sampled panoramas once, at a working resolution: 0.25 deg/px
    # is far finer than the blob centroid or the boat's pitch, and it keeps
    # the whole stack in memory so the rig median is one pass.
    loaded = []
    ...
    for frame, when, az_true, el_true, grey in loaded:
        ...
        rows.append({
            ...
            "utc": when.isoformat(),
            "course_deg": round(float(course), 2),
            ...
            # offset = course + az_camera_sun - az_world_sun.
            "offset_deg": round(float(course + az_camera - az_true) % 360.0,
                                2),
        })
    ...
    if not rows:
        raise SystemExit(
            ...
            "the sky is overcast or the sun is behind the vehicle's own "
            "structure.")
    ...
    verdict, detail, usable = assess(concentration, r_fixed, course_r)
    ...
    record = {"dataset": paths.dataset,
              ...
              "course_concentration_R": round(float(course_r), 4),
              ...
              "elevation_tolerance_deg": args.elevation_tolerance_deg,
              "work_width": args.work_width,
              "mask_rig": args.mask_rig,
              "frames": rows}
    out = write_sidecar(args.run_dir, record)
    print(f"\nwrote {out}")
