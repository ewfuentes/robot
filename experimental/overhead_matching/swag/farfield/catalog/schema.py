...
dict and drop the nulls. Storing the tags as one dict column makes cost
proportional to the tag values that actually exist (~35 GB / 8 min -> 2.9 GB /
31 s on that extract).

Every feather written before the change is wide, so readers must handle both.
This module is the ONLY feather reader in the farfield tree: use `read_frame`
plus `tag_dicts` / `row_dicts` rather than touching columns directly, and both
layouts work. A private reader once reported 13,210 landmarks as 16 distinct
signatures with nothing raised; going through one reader makes a schema change
break loudly in one place.
"""
...
def build_frame(ids, geometries, landmark_types, tags, crs="EPSG:4326"):
    ...
    Kept here so the writers (OSM and ENC extraction) cannot drift apart.
    """
...
def widen(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    Only for tools that genuinely need the old layout. This reintroduces the
    columns x rows cost, so it is deliberately explicit rather than something
    a reader does implicitly.
    """
...
def read_frame(path: Path) -> pd.DataFrame:
    ...
    The one sanctioned reader: geometry comes back as shapely objects with CRS
    metadata intact, never as raw WKB bytes for callers to decode themselves.
    """
    import geopandas as gpd
