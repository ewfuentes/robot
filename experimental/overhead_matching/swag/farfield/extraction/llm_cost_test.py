...
from experimental.overhead_matching.swag.farfield.extraction import llm_cost as lc
...
class ModelRatesTest(unittest.TestCase):
    ...
    def test_fallback_rates_are_the_ceiling_of_the_table(self):
        # The fallback exists to over-estimate unknown models, never to price
        # them cheaply; that only holds while DEFAULT_RATE_MODEL is the most
        # expensive entry in the table. If a dearer model is ever added, the
        # fallback must move with it.
        fallback = lc.MODEL_RATES[lc.DEFAULT_RATE_MODEL]
        for name, entry in lc.MODEL_RATES.items():
            for band in ("small", "large"):
                self.assertLessEqual(entry["input"][band],
                                     fallback["input"][band], name)
                self.assertLessEqual(entry["output"][band],
                                     fallback["output"][band], name)
