...
class TerrainTensor:
    ...
    def sample(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """Bilinear elevation at metric (x, y); border-clamped outside."""
    ...
    def sample_valid(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """Source-data validity at metric (x, y); 0 outside the grid."""
        out = torch.nn.functional.grid_sample(
