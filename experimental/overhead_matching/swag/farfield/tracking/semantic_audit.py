...
Level-1 canonicalization. For each track built by the tracking stage
(run_tracking), assemble a "dossier" (procedurally generated evidence summary
+ representative image chips) and turn it into a Vertex AI request. The model
adjudicates ONLY semantics; all geometry (support classification, context
...
from experimental.overhead_matching.swag.farfield.tracking.viz_common import (
    render_chip,  # noqa: F401 -- the ONE chip renderer, shared with keyframe_viewer
)
...
def merge_tracks(artifacts: dict):
    """({track_id: track}, {track_id: range_name}) merged across every range
    artifact of a run (keyframe_viewer.load_track_artifacts output).

    The audit artifact and its consumers (calibration.audit_io, tracklets,
    matching) key tracks by BARE track_id, so a duplicate id across range
    files is refused rather than letting one range's track silently shadow
    another's.
    """
    ...
    for range_name, artifact in artifacts.items():
        for track in artifact["tracks"]:
            ...
            if tid in tracks:
                raise SystemExit(
                    f"track_id {tid!r} appears in more than one range (in "
                    f"{range_by_track[tid]!r} and again in {range_name!r}); "
                    f"range files within a run must not share ids -- the "
                    f"audit keys tracks by bare track_id")
            tracks[tid] = track
...
def chip_boxes_for_entry(entry, obs, pano_w, pano_h, fov_deg):
    ...
    fov_deg is the ingest FOV (no default: it comes from the run's recorded
    ingest params, the same value the detections were unwrapped with).
    """
    det_box = geo.pano_bbox_for_observation(obs.boxes, pano_w, pano_h, fov_deg)
    rec = entry["rec"]
    mask_box = None
    mb = rec.get("mask_bbox_window")
    if mb is not None:
