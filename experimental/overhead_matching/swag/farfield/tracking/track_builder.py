...
class TrackBuilderConfig:
    ...
    verbatim into run_meta.json and every tracks_*.json (via
    dataclasses.asdict), which is what keeps the values visible rather than
    stale-invisible. Readers reconstruct with TrackBuilderConfig(**stored) --
    never a fresh default instance.
    """

    # Width in pixels of the panoramas these thresholds were applied to. No
    # default on purpose: set from the actual imagery by the stage (see the
    # class docstring).
...
class TrackBuilder:
    ...
    def _box_in_window(self, pano_box, origin):
        x0, y0 = origin
        rel_x = geo.signed_x_offset(pano_box[0], x0, self.pano_w)
    ...
    def _mask_in_window(self, track: Track, new_origin, new_size: int):
        ...
        dx = int(round(geo.signed_x_offset(old_x0, new_origin[0], self.pano_w)))
    ...
    def _pair_overlap(self, ta: Track, tb: Track):
        ...
        dx = int(round(geo.signed_x_offset(bx0, ax0, self.pano_w)))
