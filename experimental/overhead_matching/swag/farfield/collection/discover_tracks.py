...
from experimental.overhead_matching.swag.farfield.collection.farfield_regions import REGIONS, geometries
from experimental.overhead_matching.swag.farfield.collection.vector_tiles import (
...
def emit_registry_stanza(candidate: dict, region: str, scored: dict = None) -> str:
    ...
    `osm` is left as a TODO rather than guessed: the collection orchestrator
    refuses to build a partial catalog, and
    `//experimental/overhead_matching/swag/farfield/collection:pbf_coverage
...
def main():
    ...
    candidates = filter_candidates(
        ...
        min_quality=args.min_quality if args.min_quality is not None else 0.0)
    ...
    if args.output:
        payload["provenance"] = provenance_record(
            generator="//experimental/overhead_matching/swag/farfield/"
                      "collection:discover_tracks",
            inputs={"region": region_name, "bbox": ",".join(str(v) for v in bbox)},
            config={"zoom": args.zoom, "min_length_km": args.min_length_km,
                    "pano_only": args.pano_only,
                    "include_foot": args.include_foot,
                    "after_year": args.after_year,
