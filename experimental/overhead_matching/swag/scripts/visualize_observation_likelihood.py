...
def create_app(vigor_dataset, all_similarity, landmark_obs_calculator, obs_config, device):
    ...
    app.layout = html.Div([
        ...
        html.Div([
            html.Div([
                html.H3("Satellite Patch Similarity Heatmap"),
                dcc.Graph(id='similarity-map', style={'height': '600px'}),
            ], style={'width': '48%', 'display': 'inline-block', 'verticalAlign': 'top'}),

            html.Div([
                html.H3("Observation Log-Likelihood"),
                dcc.Graph(id='likelihood-map', style={'height': '600px'}),
            ], style={'width': '48%', 'display': 'inline-block', 'verticalAlign': 'top'}),
        ]),
    ...
    def update_stale_indicator(last_params, likelihood_mode, sigma_sat, sigma_osm, osm_view_mode, selected_pano_lm):
    ...
    @app.callback(
        [Output('similarity-map', 'relayoutData'),
         Output('likelihood-map', 'relayoutData')],
        [Input('similarity-map', 'relayoutData'),
         Input('likelihood-map', 'relayoutData')],
        prevent_initial_call=True
    )
    def sync_map_viewports(sim_relayout, like_relayout):
        """Synchronize viewports between similarity and likelihood maps."""
        ...
        # Whichever map changed, copy its viewport to the other
        if triggered_id == 'similarity-map' and sim_relayout:
            return no_update, sim_relayout
        elif triggered_id == 'likelihood-map' and like_relayout:
            return like_relayout, no_update

        return no_update, no_update

    @app.callback(
        [Output('similarity-map', 'figure'),
         Output('likelihood-map', 'figure'),
        ...
        [State('panorama-dropdown', 'data'),
         ...
         State('similarity-map', 'relayoutData'),
         State('osm-similarity-threshold', 'value')]
    )
    def update_visualization(n_clicks, osm_view_mode, selected_pano_lm, pano_idx, likelihood_mode, sigma_sat, sigma_osm, enable_likelihood, sim_relayout_data, osm_threshold):
        import time
        from dash import callback_context, no_update
        ...
        start = time.time()
        if sim_relayout_data and 'xaxis.range[0]' in sim_relayout_data:
            lon_min = sim_relayout_data['xaxis.range[0]']
            lon_max = sim_relayout_data['xaxis.range[1]']
            lat_min = sim_relayout_data['yaxis.range[0]']
            lat_max = sim_relayout_data['yaxis.range[1]']
        else:
        ...
        # Create similarity heatmap figure
        start = time.time()
        sim_fig = Figure()

        # Plot satellite patches colored by similarity
        sim_fig.add_trace(Scattergl(
        ...
        sim_fig.update_layout(
            ...
            margin=dict(r=100),
            uirevision='similarity'  # Maintain view state
        )
        timing_info['Create similarity figure'] = time.time() - start
        ...
        if likelihood_enabled:
            ...
            if cache_key in likelihood_cache:
                ...
                with torch.no_grad():
                    log_likelihoods = temp_calculator.compute_log_likelihoods(
                        particles, [pano_id_stem]).squeeze(0).numpy()
            ...
            # Create likelihood figure (whether from cache or newly computed)
            like_fig = Figure()

            # Add heatmap
            like_fig.add_trace(Heatmap(
            ...
            like_fig.update_layout(
                ...
                legend=dict(x=0, y=1, xanchor='left', yanchor='top'),
                margin=dict(r=80),
                uirevision='likelihood'  # Maintain view state
            )
            timing_info['Create likelihood figure'] = time.time() - start
        else:
            # Likelihood disabled - show placeholder
            like_fig = Figure()
            like_fig.add_annotation(
        ...
        # Add OSM landmarks to similarity figure only
        ...
        if len(osm_lats) > 0:
            osm_trace = Scattergl(
                ...
                marker=dict(
                    ...
                    colorscale='Plasma',
                    colorbar=dict(title='OSM Similarity', x=1.15, len=0.5, y=0.25),
                    size=10,
                    symbol='diamond',
        ...
        return sim_fig, like_fig, stats_html, "", current_params
