"""Download NOAA ENC (Electronic Navigational Chart) cells for landmark extraction.

NOAA distributes free S-57 vector chart cells, updated every weekday, as one ZIP
per cell at https://charts.noaa.gov/ENCs/<CELL>.zip. Each ZIP unpacks to
ENC_ROOT/<CELL>/<CELL>.000 (base file) plus incremental updates (.001, .002, ...)
which GDAL's S-57 driver applies automatically on read.

Cells can be named explicitly, or selected by intersecting a bounding box against
a state's ISO-19115 product catalog (https://charts.noaa.gov/ENCs/<ST>_ENCProdCat_19115.xml).
Cell names encode the usage band in the third character (1=Overview ... 5=Harbor,
6=Berthing); band 5 is the right scale for harbor-sized areas.

The eight band-5 cells covering Boston Harbor (the boston_harbor_dataset ferry legs):
    US5BOSCC US5BOSCD US5BOSCE US5BOSDB US5BOSDC US5BOSDD US5BOSBE US5BOSBF

Example:
    bazel run //experimental/overhead_matching/swag/scripts:download_enc_cells -- \\
...
DEFAULT_OUTPUT_DIR = Path("/data/overhead_matching/datasets/enc_cells")
...
def parse_catalog_cell_bboxes(catalog_xml: str) -> dict[str, tuple[float, float, float, float]]:
...
def select_cells_from_catalog(
    ...
    """Return cell names whose catalog extent intersects bbox (west, south, east, north)."""
    west, south, east, north = bbox
    selected = []
    for cell, (cw, cs, ce, cn) in sorted(parse_catalog_cell_bboxes(catalog_xml).items()):
...
def download_cell(
    ...
    if cell_dir.exists() and not force:
        print(f"{cell}: already present at {cell_dir}, skipping (use --force to refresh)")
        return cell_dir
    ...
    if not cell_dir.exists():
        raise FileNotFoundError(
            f"{cell}: ZIP did not contain ENC_ROOT/{cell}/ (unexpected layout)"
        )
...
def main(
    ...
    if cells is None:
        if catalog_state is None or bbox is None:
            raise ValueError("Provide either --cells, or --catalog_state with --bbox")
        ...
        (catalog_dir / f"{catalog_state}_ENCProdCat_19115.xml").write_text(catalog_xml)
        cells = select_cells_from_catalog(catalog_xml, bbox, band)
        print(f"Catalog selection (band={band}, bbox={bbox}): {len(cells)} cells: {cells}")

    output_dir.mkdir(parents=True, exist_ok=True)
    return [download_cell(cell, output_dir, force=force, fetch_fn=fetch_fn) for cell in cells]
...
if __name__ == "__main__":
    ...
    group.add_argument("--cells", nargs="+", help="Explicit ENC cell names (e.g. US5BOSCD)")
    group.add_argument("--catalog_state", help="Two-letter state code for catalog-based selection")
    ...
    parser.add_argument("--band", type=int, default=5,
                        help="Usage band filter for catalog selection (default 5=Harbor; "
                             "pass --band -1 to disable)")
    parser.add_argument("--output_dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--force", action="store_true", help="Re-download existing cells")
