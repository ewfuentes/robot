...
def build_dataset_to_tasks_explicit_mode(
    ...
    Args:
        ...
        landmark_correspondence_inflation_factor: Inflation factor for landmark correspondence
...
def main(train_config_paths: list[Path],
         ...
         landmark_correspondence_inflation_factor: float | None = None,
    ...
    for (ds_path, lv, radius, inflation_factor), tasks in dataset_to_tasks.items():
        ...
        print(f"  - {ds_path.name} (landmark_version={lv}, radius={radius}, inflation_factor={inflation_factor}): {len(tasks)} task(s) from {num_configs} config(s)")
    ...
    for (ds_path, lv, radius, inflation_factor), tasks in dataset_to_tasks.items():
        ...
        print(f"  landmark_version={lv}, panorama_landmark_radius_px={radius}, landmark_correspondence_inflation_factor={inflation_factor}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--landmark_correspondence_inflation_factor', type=float, default=1.0,
                        help='Inflation factor for landmark-to-satellite correspondence matching. '
                             'Values > 1.0 expand the matching region. (default: 1.0)')
