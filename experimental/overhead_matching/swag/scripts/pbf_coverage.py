...
almost everything -- the exact failure this module exists to prevent.
    ...
    python pbf_coverage.py europe/france/nord-pas-de-calais-latest.osm.pbf
    python pbf_coverage.py --bbox W S E N <spec> [<spec> ...]
"""
...
POLY_CACHE = Path.home() / "scratch" / "osm_downloads" / "poly"
...
def fetch_poly(spec: str, cache_dir: Path = POLY_CACHE) -> Path:
...
def check_coverage(specs, bbox, tolerance_frac=COVERAGE_TOLERANCE_FRAC,
                   cache_dir: Path = POLY_CACHE, reference_specs=None):
    ...
    reported for information, never failed on. With no reference given, the
    absolute number is all we have, so it is reported and only a *total* miss
    fails.
...
def load_index(cache_dir: Path = POLY_CACHE):
...
def leaf_regions(cache_dir: Path = POLY_CACHE):
...
def mapped_area(bbox, cache_dir: Path = POLY_CACHE):
...
def suggest_extracts(bbox, cache_dir: Path = POLY_CACHE, max_mb: float = 700.0):
...
def main():
    ...
    if args.suggest:
        if not args.bbox:
            ...
            return 2
        chosen, frac = suggest_extracts(tuple(args.bbox))
