...
class LambdaCloudClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError("API key must be provided via parameter or LAMBDA_API_KEY environment variable")
    ...
    def launch_instance_with_retry(self,
        ...
        for attempt in range(max_retries + 1):
            try:
                if callback:
                    callback(f"Attempt {attempt + 1}/{max_retries + 1}: Launching {quantity}x {instance_type_name} in {region_name}")
                ...
                try:
                    ...
                    if callback:
                        if not available_types:
                            callback(f"No capacity in {region_name}. Available regions for {instance_type_name}: checking...")
                            # Find regions with this instance type
                            matching_types = [t for t in instance_types if t.name == instance_type_name]
                            if matching_types:
                                available_regions = matching_types[0].regions
                                callback(f"Available regions for {instance_type_name}: {', '.join(available_regions)}")
                        else:
                            available_names = [t.name for t in available_types]
                            callback(f"No capacity for {instance_type_name} in {region_name}. Available types: {', '.join(available_names)}")
        ...
        raise InsufficientCapacityError("max_retries_exceeded", f"Failed to launch after {max_retries + 1} attempts")
