#!/usr/bin/env python3
"""Training script for simplified landmark correspondence classifier.

Uses SimpleCorrespondenceClassifier with all-text encoding and 4 cross features.

Usage:
    bazel run //experimental/overhead_matching/swag/scripts:train_simple_correspondence -- \
...
from experimental.overhead_matching.swag.data.simple_correspondence_dataset import (
    PARSE_REPORT_PATH,
    SimpleLandmarkCorrespondenceDataset,
...
def compute_metrics(
    ...
    return {"auc_roc": float(auc), "precision": float(prec),
...
def train(config: CorrespondenceTrainConfig) -> None:
    ...
    train_dataset = SimpleLandmarkCorrespondenceDataset(
        train_pairs, text_embeddings, text_input_dim, include_difficulties,
    )
    val_dataset = SimpleLandmarkCorrespondenceDataset(
    ...
    train_loader = DataLoader(
        ...
        collate_fn=collate_correspondence, num_workers=config.num_workers,
    ...
    # Create model
    encoder_config = SimpleTagBundleEncoderConfig(
        text_input_dim=text_input_dim, text_proj_dim=config.encoder.text_proj_dim,
    )
    classifier_config = SimpleCorrespondenceClassifierConfig(
        encoder=encoder_config,
        mlp_hidden_dim=config.classifier.mlp_hidden_dim,
        dropout=config.classifier.dropout,
    )
    model = SimpleCorrespondenceClassifier(classifier_config).to(device)
    ...
    best_auc = 0.0
    print(f"\nTraining for {config.num_epochs} epochs ({total_steps} steps, {warmup_steps} warmup)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train simplified landmark correspondence classifier")
