...
class DeriveIncrementsTest(unittest.TestCase):
    def test_straight_track_east(self):
        ...
        north = np.zeros(5)
        increments = gps_to_odometry.derive_increments(east, north)
    ...
    def test_noise_injection_declares_itself(self):
        ...
        north = np.zeros(20)
        clean = gps_to_odometry.derive_increments(east, north)
        noisy = gps_to_odometry.derive_increments(
            east, north, extra_sigma_m=3.0, extra_yaw_sigma_deg=2.0,
            noise_seed=7)
    ...
    def test_deterministic(self):
        ...
        a = gps_to_odometry.derive_increments(east, north, extra_sigma_m=1.0)
        b = gps_to_odometry.derive_increments(east, north, extra_sigma_m=1.0)
        self.assertEqual(a, b)

    def test_rejects_bad_input(self):
        with self.assertRaises(ValueError):
            gps_to_odometry.derive_increments([0.0], [0.0])
        with self.assertRaises(ValueError):
            gps_to_odometry.derive_increments([0.0, 1.0], [0.0, 1.0],
                                              sigma_pair_m=0.0)
...
class RewriteExportTest(unittest.TestCase):
    def _make_export(self, root: Path) -> Path:
        ...
        (export_dir / "export_meta.json").write_text(json.dumps(
            {"schema_version": "0.1", "scenario_name": "tiny",
        ...
        (export_dir / "tier1_odometry.jsonl").write_text(
            '{"keyframe_idx": 1, "dx_m": 0.0}\n')  # legacy row
        return export_dir

    def test_rewrites_into_sibling_and_preserves_source(self):
        with tempfile.TemporaryDirectory() as tmp:
            export_dir = self._make_export(Path(tmp))
            output_dir = Path(tmp) / "export_v02"
            increments = gps_to_odometry.rewrite_export(export_dir,
                                                        output_dir)
            self.assertEqual(len(increments), 5)

            written = [
                msgspec.json.decode(line, type=structs.OdometryDelta,
                                    dec_hook=msgspec_dec_hook)
                for line in (output_dir / "tier1_odometry.jsonl")
                .read_bytes().splitlines()]
            self.assertEqual(written, increments)
            meta = json.loads((output_dir / "export_meta.json").read_text())
            self.assertEqual(meta["schema_version"], structs.SCHEMA_VERSION)
            # Fields this build does not model must survive the round trip.
            self.assertTrue(meta["custom_field_this_build_ignores"])
            # The source export is untouched, legacy odometry included.
            self.assertIn("dx_m",
                          (export_dir / "tier1_odometry.jsonl").read_text())
            self.assertEqual(json.loads(
                (export_dir / "export_meta.json").read_text())
                ["schema_version"], "0.1")
    ...
    def test_refuses_in_place(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "in place"):
                gps_to_odometry.rewrite_export(export_dir, export_dir)
