...
three things a static file structurally cannot have:
  ...
  POST /replay               a live counterfactual: apply `Edits`, run the
                             ...
                             Writes under /tmp, because a what-if is a question
                             and questions should not accumulate next to the
                             runs they were asked about.
...
Usage:
  bazel run //experimental/overhead_matching/swag/bearing_only_localization:viewer_server -- \\
...
def create_app(run_dir: Path, sources_dir: Path | None = None,
               ...
               max_visible_range_m: float | None = None) -> Flask:
    app = Flask(__name__)
    state = {
        "payload": None,
        "checkpoints": None,
    ...
    def checkpoints() -> dict:
        if state["checkpoints"] is None:
            state["checkpoints"] = run_log.read_run(run_dir).checkpoints
    ...
    def api_replay():
        ...
        try:
            result = replay_mod.replay(
                run_dir, edits=edits,
                max_visible_range_m=max_visible_range_m)
            output_dir = (DEFAULT_COUNTERFACTUAL_BASE / Path(run_dir).name
                          / edits.slug())
