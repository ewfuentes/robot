...
class DerivedEventTest(unittest.TestCase):
    ...
    def _data(self, health_records, proposal_events=(), mode_events=(),
              n_particles=1000):
        manifest = structs.RunManifest(
            schema_version=structs.SCHEMA_VERSION, scenario_name="events",
            anchor_lat_deg=42.0, anchor_lon_deg=-71.0,
            n_keyframes=len(health_records),
            filter_config=structs.FilterConfig(
                n_particles=n_particles, seed=0,
                init=structs.GaussianInit(0.0, 0.0, 100.0)),
            landmarks=[], matcher_version="test")
        return run_log.RunData(
            manifest=manifest, truth=[], odometry=[], measurements=[],
