...
def _table_lookup(table, landmark_id: str):
    ...
    for entry in table.entries:
        if entry.landmark_id == landmark_id:
            ...
            return clipped, clipped > default + 1e-12
    return default, False


def _endorsed_entries(table) -> dict:
...
def _residual_matrix(epochs, truth_by_kf, catalog):
    ...
    observed = np.array([
        geodesy.wrap_rad(
            math.radians(truth_by_kf[m.anchor_keyframe_idx].heading_deg)
            + math.radians(m.bearing_body_deg))
        for m in epochs])
    residual = np.abs(np.degrees(geodesy.wrap_rad(
        bearings - observed[:, None])))
...
def _fit(landmark_id, residuals, ranges, index, table) -> CandidateFit:
    ...
    log_lr, endorsed = _table_lookup(table, landmark_id)
    return CandidateFit(
...
def _triage_tracklet(tracklet_id, epochs, truth_by_kf, catalog, table,
    ...
    endorsed = _endorsed_entries(table)
    endorsed_fits = [
    ...
    for row, meas in enumerate(epochs):
        ...
        epoch_rows.append(EpochTriage(
            ...
            true_world_bearing_deg=(
                (truth_by_kf[meas.anchor_keyframe_idx].heading_deg
                 + meas.bearing_body_deg) % 360.0),
