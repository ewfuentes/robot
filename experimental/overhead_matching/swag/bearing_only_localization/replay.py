...
# What `LandmarkCatalog` would pick on its own. Only used when a manifest
# predates `max_visible_range_m` AND the caller supplied nothing, and the
# choice is reported as an assumption rather than applied silently.
FALLBACK_MAX_VISIBLE_RANGE_M = 15000.0
...
def replayability(run_dir: Path) -> Replayability:
    ...
    if not has_range:
        notes.append(
            ...
            f"geometry will assume {FALLBACK_MAX_VISIBLE_RANGE_M:.0f} m "
...
def region_frame(manifest: structs.RunManifest) -> geodesy.RegionFrame:
    return geodesy.RegionFrame(manifest.anchor_lat_deg, manifest.anchor_lon_deg)


def build_catalog(manifest: structs.RunManifest, max_visible_range_m: float,
                  frame: geodesy.RegionFrame | None = None
...
def load_inputs(run_dir: Path, max_visible_range_m: float | None = None,
                data: run_log.RunData | None = None) -> ReplayInputs:
    """Rebuild the filter's inputs from a run directory."""
    run_dir = Path(run_dir)
    data = data if data is not None else run_log.read_run(run_dir)
    visible_range = (max_visible_range_m
                     if max_visible_range_m is not None
                     else data.manifest.max_visible_range_m)
    if visible_range is None:
        visible_range = FALLBACK_MAX_VISIBLE_RANGE_M
...
def replay(run_dir: Path, edits: Edits | None = None,
           ...
           verify: bool = True,
           data: run_log.RunData | None = None) -> ReplayResult:
    ...
    inputs = load_inputs(run_dir, max_visible_range_m, data=data)
...
def write_counterfactual(output_dir: Path, source_run_dir: Path,
    ...
    run_log.write_run(output_dir, manifest, result.inputs.data.truth,
