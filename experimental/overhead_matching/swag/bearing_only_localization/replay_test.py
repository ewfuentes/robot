...
def _write_run(run_dir: Path, config: structs.FilterConfig, data,
               tables=None, max_visible_range_m: float | None = 10000.0):
...
class ReplayFidelityTest(unittest.TestCase):
    ...
    def test_recorded_visible_range_is_used(self):
        """The radius feeds proposal geometry, so replay must not guess it."""
        with tempfile.TemporaryDirectory() as tmp:
            ...
            np.testing.assert_allclose(inputs.catalog.max_visible_range_m,
                                       10000.0)
            self.assertEqual(inputs.max_visible_range_m, 10000.0)

    def test_missing_visible_range_is_reported_not_assumed(self):
        with tempfile.TemporaryDirectory() as tmp:
            run_dir, _, _, _ = _harbor_run(Path(tmp),
                                           )
            ...
            self.assertTrue(any("max_visible_range_m" in note
