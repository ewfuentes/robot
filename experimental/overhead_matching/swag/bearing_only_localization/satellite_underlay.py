...
imagery can be pinned near a dataset's own capture date. Construction moves in a
harbour, and an underlay from three years later is a misleading backdrop for a
matcher argument.
...
for a backdrop, not a survey.
    ...
    bazel run //experimental/overhead_matching/swag/bearing_only_localization:satellite_underlay -- \\
...
def find_release(date: str | None, session) -> tuple[int, str]:
    """(release number, label) for the Wayback release nearest `date`.

    `date` is YYYY-MM or YYYY-MM-DD. Without one, the most recent release.
    """
    config = session.get(WAYBACK_CONFIG_URL, timeout=30).json()
    releases = []
    for entry in config.values():
        url = entry.get("itemURL") or entry.get("itemTitle") or ""
    ...
    if date is None:
        # By DATE, not by release number: Wayback numbers are not chronological.
        # Picking the largest number returned the 2023-08-31 release as if it
        # were current, which is a silently three-year-old backdrop.
        best = max(releases, key=lambda r: stamp(r[1]))
...
def main():
    ...
    parser.add_argument("--date", default=None,
                        help="YYYY-MM: pin imagery near the capture date")
    ...
    frame = geodesy.RegionFrame(manifest.anchor_lat_deg, manifest.anchor_lon_deg)
