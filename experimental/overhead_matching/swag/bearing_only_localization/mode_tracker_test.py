...
class FilterIntegrationTest(unittest.TestCase):
    def test_symmetric_world_holds_two_modes_then_collapses(self):
        """T-F3 with the modes named rather than inferred from a
        mass-on-each-side count — which a single cloud straddling the axis
        passes trivially.

        The required behaviour is not "stay bimodal": it is hold both
        hypotheses *until evidence kills one*, then commit to the right one.
        Premature collapse is the unrecoverable failure; never collapsing
        would be a filter that cannot use its own evidence."""
