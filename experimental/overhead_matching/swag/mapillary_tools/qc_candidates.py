...
"""GPS-consistency and image-density QC for candidate trajectories.
    ...
    bazel run //experimental/overhead_matching/swag/mapillary_tools:qc_candidates -- --seeds 957814683411464,548801286115301
    bazel run //experimental/overhead_matching/swag/mapillary_tools:qc_candidates -- --local /data/farfield_matching/datasets/seattle
    bazel run //experimental/overhead_matching/swag/mapillary_tools:qc_candidates -- --seeds ... --output qc.json
...
# --------------------------------------------------------------------------
# geometry helpers on bare (lat, lng) tuples so the same code serves API
# images and local CSV rows
# --------------------------------------------------------------------------
...
def motion_consistency(points: list[tuple], times_ms: list = None) -> dict:
    ...
    """
    lat0, lng0 = points[0]
    xy = [_xy_m(lat, lng, lat0, lng0) for lat, lng in points]
