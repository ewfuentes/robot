...
pano_id_mapping.csv, frames_gps.csv, intrinsics.csv and extraction_log.csv.
...
def load_sequence_metadata(sequence_dir: Path) -> list[dict]:
    """Load all JSON metadata files from a Mapillary sequence directory.

    Returns list of dicts sorted by sequence_position (authoritative spatial
    order from Mapillary API). Falls back to captured_at if sequence_position
    is not available (legacy data).
    """
    metadata = []
    ...
    if has_seq_pos:
        ...
        print("  (Re-download with sequences.py --download for correct ordering)")
    ...
    if dupes:
        ...
        if not has_seq_pos:
            print(f"  This WILL cause ordering errors. Re-download with sequence_position to fix.")
...
def gps_to_local_meters(lat, lon, ref_lat, ref_lon):
    """Convert lat/lon to local x (east), y (north) in meters relative to ref point."""
    x = (lon - ref_lon) * math.radians(1) * EARTH_R * math.cos(math.radians(ref_lat))
    y = (lat - ref_lat) * math.radians(1) * EARTH_R
    return x, y


def gps_heading(m1, m2):
    """Compute compass heading from m1 to m2 using lat/lng."""
    dlat = m2["lat"] - m1["lat"]
    dlng = m2["lng"] - m1["lng"]
    if abs(dlat) < 1e-9 and abs(dlng) < 1e-9:
        return None
    return math.degrees(math.atan2(dlng * math.cos(math.radians(m1["lat"])), dlat)) % 360
...
def compute_bbox_and_stats(metadata: list[dict]) -> dict:
    ...
    width_km = haversine_m(mid_lat, west, mid_lat, east) / 1000
...
# ── Rig offset detection ─────────────────────────────────────────────────────


def compute_rig_offset(metadata: list[dict], heading_field: str) -> dict:
    """Auto-detect camera rig offset from GPS-derived travel heading.

    The rig offset is the angle between the selected heading (center of
    equirectangular) and the actual travel direction. A non-zero offset means
    the camera rig front is rotated relative to the vehicle's forward direction.

    Returns dict with offset_deg, std_deg, n_samples, and per-frame offsets.
    """
    offsets = []
    for i in range(len(metadata) - 1):
        gh = gps_heading(metadata[i], metadata[i + 1])
...
# Mapillary reports 360 imagery under either name. Verified by downloading one
# of each and confirming both are ordinary 2:1 equirectangular panoramas — a
# dual-fisheye layout would also be 2:1, so aspect ratio alone is not proof.
# They differ only in the string; no reprojection is needed for either.
...
def repair_implausible_focals(metadata, verbose=True):
    ...
    Returns {mapillary_id: replacement_focal} for the frames that need one.
...
def process_single_image(args):
    ...
    Returns (idx, output_filename, error_msg).
    """
    (idx, meta, output_dir, jpeg_quality, rig_offset, target_width,
     is_equirect, pano_id) = args
...
def create_visualization(
    ...
    rig_offset, all_metadata, current_idx, frame_idx, total_frames,
    ...
    ax2.set_title("North-Aligned Panorama (North at Center)", fontsize=11)
    ax2.axis("off")
    ...
    for m in all_metadata:
        x, y = gps_to_local_meters(m["lat"], m["lng"], ref_lat, ref_lon)
        all_x.append(x)
        all_y.append(y)
    ...
    # Corrected heading arrow on trajectory (green = what the image was actually rolled to)
    ...
    # corrected heading (green) — what the image was actually rolled to
    ...
    ax4.set_title(
        ...
        f"corrected: {corrected_heading:.0f}° (green)",
        fontsize=10,
    )
    ax4.axis("off")
...
def main():
    ...
    parser.add_argument("--heading_source", choices=("auto", "computed", "compass"),
                        default="auto",
                        help="Heading used for the north-align roll: Mapillary's "
                             "SfM 'computed_compass_angle', the magnetometer "
                             "'compass_angle', or 'auto' (validate both against "
                             "the GPS travel bearing and pick the better one)")
    parser.add_argument("--max_heading_error_deg", type=float, default=10.0,
                        help="Fail if the selected heading source's median "
                             "disagreement with the GPS travel bearing exceeds "
                             "this (default: 10)")
    parser.add_argument("--max_perspective_offset_std_deg", type=float, default=45.0,
                        help="Report a perspective capture as hand-held rather than "
                             ...
                             "(default: 45)")
    parser.add_argument("--max_heading_source_disagreement_deg", type=float, default=25.0,
                        help="Warn when a perspective capture's two heading sources "
                             "(SfM vs magnetometer) disagree by more than this median "
                             "amount, meaning at least one is wrong (default: 25)")
    ...
    parser.add_argument("--rig_offset", default="auto",
                        help="Rig offset in degrees (default: auto-detect from GPS). "
                             "Corrects for camera rigs mounted sideways.")
    parser.add_argument("--jpeg_quality", type=int, default=95, help="JPEG output quality")
    parser.add_argument("--resize", type=int, default=None,
                        help="Resize to this max width (e.g. 5760). Maintains aspect ratio.")
    parser.add_argument("--num_workers", type=int, default=NUM_WORKERS,
                        help="Number of parallel workers")
    parser.add_argument("--trim_start", type=float, default=0,
                        help="Trim first N%% of trajectory (e.g. 50 = keep last half)")
    parser.add_argument("--trim_end", type=float, default=0,
                        help="Trim last N%% of trajectory")
    parser.add_argument("--min_spacing", type=float, default=0,
    ...
    if args.min_spacing > 0:
        filtered = [metadata[0]]
        for m in metadata[1:]:
            d = haversine_m(filtered[-1]["lat"], filtered[-1]["lng"], m["lat"], m["lng"])
    ...
    if not is_equirect:
        ...
        print("  No north-align roll will be applied — rolling a perspective image "
              "is meaningless. Per-frame heading + intrinsics go to intrinsics.csv.")
    ...
    print(f"  (Reference: Chicago 29 km², Seattle 27 km², NewYork 58 km², "
          f"SanFrancisco 32 km², Boston 61 km²)")
    ...
    if args.heading_source != "auto":
        ...
        if zero_frac > 0.2:
            ...
            print(f"  Auto-selected: computed_compass_angle "
                  f"(SfM-derived; preferred over the magnetometer for "
                  f"perspective captures)")
    ...
    if not heading_gate_applies:
        ...
        # offset is *consistent* (a fixed mount), which the rig-offset std below
        ...
        print(f"  Perspective capture: skipping the GPS-bearing gate "
              ...
              f"Consistency is checked via the rig-offset spread below.")
    ...
    # ── Step 3b: Rig offset ──
    print("\n[3/6] Rig offset detection...")
    offset_info = compute_rig_offset(metadata, heading_field)

    if args.rig_offset == "auto":
        rig_offset = offset_info["offset_deg"]
        print(f"  Auto-detected: {rig_offset:+.1f}° "
              f"(std: {offset_info['std_deg']:.1f}°, "
              f"n={offset_info['n_samples']} pairs)")
    else:
        rig_offset = float(args.rig_offset)
        print(f"  Manual override: {rig_offset:+.1f}°")
        print(f"  (auto-detect would give: {offset_info['offset_deg']:+.1f}° "
              f"±{offset_info['std_deg']:.1f}°)")

    if abs(rig_offset) < 5:
        print(f"  Camera rig front ≈ travel direction (no correction needed)")
    else:
        print(f"  Camera rig front is {rig_offset:+.1f}° from travel direction")
        print(f"  (diagnostic only — the roll uses {heading_field} directly)")
    ...
    # landmark_filtering/ingest.py relies on when it sorts frames by pano_id.
    ...
    print(f"  Rig offset: {rig_offset:+.1f}°")
    if args.resize:
        print(f"  Resizing to max width: {args.resize}")

    work_items = [
        (i, meta, frames_dir, args.jpeg_quality, rig_offset, args.resize,
         is_equirect, meta["pano_id"])
        for i, meta in enumerate(metadata)
    ]
    ...
    # frames_gps.csv — the per-frame table the farfield landmark_filtering
    # pipeline reads (idx, dist_m, video_t_s). Nothing else in either repo
    # writes this file; the schema mirrors boston_harbor_dataset/processed/legN.
    # There is no video here, so video_t_s is seconds since the first capture,
    # which is what the frame-indexing code needs it to mean.
    ...
    with open(gps_path, "w", newline="") as f:
        ...
        for out_idx, (i, meta) in enumerate(kept):
            if out_idx > 0:
                ...
                cumulative_m += haversine_m(prev["lat"], prev["lng"],
                                            meta["lat"], meta["lng"])
                dt = (meta["captured_at"] - prev["captured_at"]) / 1000.0
                step = haversine_m(prev["lat"], prev["lng"], meta["lat"], meta["lng"])
                # -1 marks "undefined", the convention boston_harbor uses when
                # the platform is stationary or the interval is degenerate.
    ...
    if True:
        ...
        with open(intrinsics_path, "w", newline="") as f:
            ...
            for out_idx, (i, meta) in enumerate(kept):
                ...
                stored_w = min(meta["width"], args.resize) if args.resize else meta["width"]
    ...
    # boston_harbor_dataset uses absolute link targets and silently breaks
    # every image read when relocated.
    ...
    # Capture date from first image's captured_at (epoch ms)
    captured_at_ms = metadata[0]["captured_at"]
    from datetime import datetime, timezone
    capture_date = datetime.fromtimestamp(captured_at_ms / 1000, tz=timezone.utc).strftime("%Y-%m-%d")

    # Provenance that downstream consumers need in order to interpret the
    # images at all: whether column-to-azimuth holds (is_equirectangular),
    # which frames are north-aligned, and where the positions came from.
    ...
    pipeline_meta = {
        ...
        "north_aligned": False,
        # Machine-readable so no consumer has to guess. Images are stored
        ...
        "heading_reliable": bool(
            ...
            and sel["median_err_deg"] <= args.max_heading_error_deg
        ) if is_equirect else None,
        "heading_sources_median_disagreement_deg": (
            round(statistics.median([_circ_err(m["computed_compass_angle"],
                                               m["compass_angle"])
                                     for m in metadata]), 2) if metadata else None),
        "heading_sources_disagree": (
            None if is_equirect else bool(
                statistics.median([_circ_err(m["computed_compass_angle"],
                                             m["compass_angle"])
                                   for m in metadata])
        ...
        "camera_pans_relative_to_travel": (
            None if is_equirect else bool(
                ...
                and offset_info["std_deg"] > args.max_perspective_offset_std_deg)),
        "max_heading_error_deg": args.max_heading_error_deg,
        "heading_validation_overridden": bool(args.skip_heading_validation),
        "rig_offset_deg": rig_offset,
        "rig_offset_auto": offset_info,
