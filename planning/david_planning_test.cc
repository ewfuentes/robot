...
/*
    TEST(DavidPlannerTest, testing_plans){
        ...
        const DavidPlannerConfig david_config{
            ...
            .max_states_per_path = 50,
            .favor_goal = 0.7,
            .favor_exploration = 1 - david_config.favor_goal,
            .edge_cost = 5.0,
        ...
        const auto maybe_plan = david_planner<int>(successor_func,termination_check,road_map,david_config,est,beacon_potential,ekf_config);
...
*/
    TEST(DavidPlannerTest, testing_evaluation){
        ...
        const double P_SECOND_BEACON = 0.9;
        const auto &[grid, ekf_slam, potential] = create_david_grid_environment(ekf_config,P_FIRST_BEACON,P_SECOND_BEACON);
        const auto road_map = grid;
        const auto beacon_potential = potential;
        ...
        // ACTION
        const auto maybe_plan = david_planner<int>(successor_func,termination_check,road_map,david_config,est,beacon_potential,ekf);
        //VERIFICATION

        // Printing beacons
        std::vector<int> beacon_ids = est.beacon_ids;
        std::unordered_map<int,bool> p_beacon_pass_in;
        const auto beacons = beacon_mapping<int>(successor_func,termination_check,road_map,est);
        ...
        // Printing config
        std::cout << david_config.favor_goal*100 << "% Goal oriented and " << david_config.favor_exploration*100 << "% exploratory" << std::endl;
        std::cout << "Sampled From " << david_config.max_plans << " Plans, With All Below " << david_config.max_states_per_path << " States Per Path." << std::endl;
        std::cout << std::endl;
